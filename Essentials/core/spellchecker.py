...
class TokenAnalysis:
    ...
    phase: str = ""
    recognition_sources: tuple[str, ...] = ()


@dataclass(slots=True)
class FunctionPhraseDecision:
...
class _DummyMeaningIndex:
    def meaning_for(self, word: str) -> str:
        return ""
    def meanings_for(self, word: str) -> tuple[str, ...]:
        return ()

_DUMMY_MEANING_INDEX = _DummyMeaningIndex()
...
MALTESE_USAGE_DIC = FINAL_DICS_DIR / "Maltese_usage.dic"
...
SYMSPELL_BLOCKING_BUILD = os.environ.get(
    "SPELLCHECK_SYMSPELL_BLOCKING_BUILD",
    "false",
).lower() in {"1", "true", "yes", "on"}
...
ENABLE_APERTIUM_ANALYZER = os.environ.get(
    "SPELLCHECK_ENABLE_APERTIUM",
    "false",
).lower() in {"1", "true", "yes", "on"}
APERTIUM_TIMEOUT_SEC = float(os.environ.get("SPELLCHECK_APERTIUM_TIMEOUT_SEC", "1.5"))
ENABLE_BULK_TOKEN_PREWARM = os.environ.get(
    "SPELLCHECK_BULK_TOKEN_PREWARM",
    "false",
).lower() in {"1", "true", "yes", "on"}

_configured_corpus_mode = os.environ.get("SPELLCHECK_CORPUS_MODE", "").strip().lower()
if not _configured_corpus_mode:
    _legacy_corpus_enabled = os.environ.get("SPELLCHECK_CORPUS_SCORING")
    if _legacy_corpus_enabled is None:
        _configured_corpus_mode = "active"
    elif _legacy_corpus_enabled.lower() in {"1", "true", "yes", "on"}:
        _configured_corpus_mode = (
            "shadow"
            if os.environ.get("SPELLCHECK_CORPUS_SHADOW", "false").lower()
            in {"1", "true", "yes", "on"}
            else "active"
        )
    else:
        _configured_corpus_mode = "off"
CORPUS_CONTEXT_MODE = (
    _configured_corpus_mode
    if _configured_corpus_mode in {"off", "shadow", "active"}
    else "off"
)
ENABLE_CORPUS_SCORING = CORPUS_CONTEXT_MODE != "off"
CORPUS_SHADOW_MODE = CORPUS_CONTEXT_MODE == "shadow"
...
CORPUS_CONTEXT_MARGIN = float(
    os.environ.get("SPELLCHECK_CORPUS_CONTEXT_MARGIN", "0.012")
)
...
class UniversalMalteseSpellchecker:
    ...
    EURO_AMOUNT_PATTERN = re.compile(
        r"(?<![\w\d])(?:"
        r"€\s*(?P<prefix>\d+(?:[.,]\d{1,2})?)\s*[cC]?"
        r"|(?P<suffix>\d+(?:[.,]\d{1,2})?)\s*€"
        r")(?![\w\d])",
        re.UNICODE,
    )
    ...
    ENGLISH_USAGE_TAG_PREFIX = "EN-MALTI-"
    MALTESE_USAGE_VERB_TAG_PREFIX = "MALTI-VERB-"
    MALTESE_USAGE_TAG_PREFIX = "MALTI-"
    ...
    MANUAL_WORD_REPAIRS = {
        ...
        "ragel": "raġel",
        "hin": "ħin",
        "dak il vitamini": "dawk il-vitamini",
        "dak il-vitamini": "dawk il-vitamini",
        "edd": "qed",
        "aw": "hawn",
        "awn": "hawn",
    ...
    EXACT_SUGGESTION_OVERRIDES = {
        ...
        "tamlu": ("tagħmlu",),
        "jamlu": ("jagħmlu",),
        "atlek": ("għedtlek", "qaltlek"),
        "atli": ("għedtli", "qaltli"),
        "atlu": ("għedtlu", "qaltlu"),
        "atlha": ("għedtlha", "qaltlha"),
        "atlna": ("għedtlna", "qaltlna"),
        "atlkom": ("għedtlkom", "qaltlkom"),
        "atlhom": ("għedtlhom", "qaltlhom"),
        "kiku": ("kieku",),
        "mien": ("minn", "min"),
    }
    ...
    def __init__(
        ...
        self._maltese_cultural_names: frozenset[str] | None = None
        ...
        self.place_phrase_shortcut_display: dict[str, str] = {}
        self.place_phrase_first_words: set[str] = set()
        ...
        self._english_usage_mappings: dict[str, list[str]] = {}
        self._maltese_usage_mappings: dict[str, list[str]] = {}
        self._maltese_usage_verb_specs: list[tuple[str, str]] = []
        self._maltese_usage_verbs_finalized = False
        ...
        self._symspell_lock = threading.Lock()
        ...
        usage_entries = self._load_dictionary_files([MALTESE_USAGE_DIC])
        self._index_usage_entries(usage_entries)
        raw_entries.extend(
            (surface, None) for surface in self._maltese_usage_mappings
        )
        ...
        self.dictionary_set = set(self.dictionary) | set(self.MANUAL_WORD_REPAIRS.values()) | set(self.MANUAL_WORD_REPAIRS.keys())
    ...
    def _index_usage_entries(
        self,
        entries: Iterable[tuple[str, str | None]],
    ) -> None:
        """Index exact usage alternatives declared by tags in the usage dictionary."""
        for surface, raw_tag in entries:
            tag = str(raw_tag or "").strip()
            if tag.startswith(self.ENGLISH_USAGE_TAG_PREFIX):
                key = self._english_key(surface)
                suggestion = tag[len(self.ENGLISH_USAGE_TAG_PREFIX) :].strip()
                target = self._english_usage_mappings
            elif tag.startswith(self.MALTESE_USAGE_VERB_TAG_PREFIX):
                source = self._normalize_word(surface)
                target_verb = tag[
                    len(self.MALTESE_USAGE_VERB_TAG_PREFIX) :
                ].strip()
                if (
                    source
                    and target_verb
                    and (source, target_verb) not in self._maltese_usage_verb_specs
                ):
                    self._maltese_usage_verb_specs.append((source, target_verb))
                continue
            elif tag.startswith(self.MALTESE_USAGE_TAG_PREFIX):
                key = self._normalize_word(surface)
                suggestion = tag[len(self.MALTESE_USAGE_TAG_PREFIX) :].strip()
                target = self._maltese_usage_mappings
            else:
                continue
            if not key or not suggestion:
                continue
            suggestions = target.setdefault(key, [])
            if suggestion not in suggestions:
                suggestions.append(suggestion)

    @staticmethod
    def _usage_record_signature(record) -> tuple[str, str, bool]:
        return (
            str(getattr(record, "tense", "")),
            str(getattr(record, "person", "")),
            str(getattr(record, "raw_tag", "")).endswith("-N"),
        )

    def _standard_nonsemitic_usage_forms(
        self,
        lemma: str,
    ) -> list[tuple[str, str, str, bool]]:
        """Generate the regular unsuffixed paradigm used by loan verbs in -ja."""
        lemma = self._normalize_word(lemma)
        if not lemma.endswith("ja"):
            return []

        stem = lemma[:-1]

        def plural(surface: str) -> str:
            return f"{surface[:-1]}aw"

        if lemma[0] in self.VOWELS:
            imperfect = {
                "3SM": f"j{lemma}",
                "1S": f"n{lemma}",
                "2S": f"t{lemma}",
        ...
        forms = [
            (lemma, "IMP", "2S", False),
            (plural(lemma), "IMP", "2P", False),
            (lemma, "PERF", "3SM", False),
            (f"{stem}ajna", "PERF", "1P", False),
            (f"{stem}ajt", "PERF", "1S", False),
            (f"{stem}ajt", "PERF", "2S", False),
            (f"{stem}ajtu", "PERF", "2P", False),
            (f"{stem}at", "PERF", "3SF", False),
            (plural(lemma), "PERF", "3P", False),
            (f"{stem}ajniex", "PERF", "1P", True),
            (f"{stem}ajtx", "PERF", "1S", True),
            (f"{stem}ajtx", "PERF", "2S", True),
            (f"{stem}ajtux", "PERF", "2P", True),
            (f"{stem}atx", "PERF", "3SF", True),
            (f"{plural(lemma)}x", "PERF", "3P", True),
            (f"{lemma}x", "PERF", "3SM", True),
        ]
        ...
        for stem_key, person in imperfect_rows:
            singular = imperfect[stem_key]
            surface = plural(singular) if person.endswith("P") else singular
            forms.append((surface, "MPERF", person, False))
            forms.append((f"{surface}x", "MPERF", person, True))

        negative_imperative = imperfect["2S"]
        forms.extend(
            [
                (f"{negative_imperative}x", "IMP", "2S", True),
                (f"{plural(negative_imperative)}x", "IMP", "2P", True),
            ]
        )
        return forms

    def _usage_source_forms(
        self,
        source: str,
        verb_index,
    ) -> list[tuple[str, str, str, bool]]:
        source_records = verb_index.word_records(source)
        anchors = [
            record
            for record in source_records
            if record.tense == "PERF" and record.person == "3SM"
        ]
        if not anchors:
            return self._standard_nonsemitic_usage_forms(source)

        anchor = anchors[0]
        records = [
            record
            for record in verb_index.by_short_tag.get(anchor.short_tag, ())
            if record.extra == anchor.extra
        ]
        rows = [
            (
                self._normalize_word(record.word),
                record.tense,
                record.person,
                str(record.raw_tag).endswith("-N"),
            )
            for record in records
        ]
        return rows

    def _usage_target_paradigm(
        self,
        target_expression: str,
        verb_index,
    ) -> tuple[str, str, list] | None:
        target_head, separator, target_tail = target_expression.partition(" ")
        target_lookup = self._normalize_word(target_head)
        anchors = [
            record
            for record in verb_index.word_records(target_lookup)
            if record.tense == "PERF" and record.person == "3SM"
        ]
        if not anchors and target_lookup.startswith("i"):
            target_lookup = target_lookup[1:]
            anchors = [
                record
                for record in verb_index.word_records(target_lookup)
                if record.tense == "PERF" and record.person == "3SM"
        ...
        anchor = min(
            anchors,
            key=lambda record: (
                self._word_distance(target_lookup, record.word),
                record.line_number,
            ),
        )
        records = list(verb_index.by_short_tag.get(anchor.short_tag, ()))
        return target_lookup, target_tail if separator else "", records

    def finalize_usage_verb_mappings(self) -> None:
        """Expand compact MALTI-VERB tags after the verb index is attached."""
        if self._maltese_usage_verbs_finalized:
            return
        ...
        tense_priority = {"PERF": 0, "MPERF": 1, "IMP": 2}
        for source, target_expression in self._maltese_usage_verb_specs:
            source_rows = self._usage_source_forms(source, verb_index)
            target_paradigm = self._usage_target_paradigm(
                target_expression,
                verb_index,
            )
            if not source_rows or target_paradigm is None:
                continue

            target_lookup, target_tail, target_records = target_paradigm
            records_by_signature: dict[tuple[str, str, bool], list] = defaultdict(list)
            for record in target_records:
                records_by_signature[self._usage_record_signature(record)].append(record)

            source_rows.sort(
                key=lambda row: (
                    row[0] != source,
                    tense_priority.get(row[1], 9),
                    row[2],
                    row[3],
                )
            )
            for source_surface, tense, person, negative in source_rows:
                if source_surface == source and (tense, person) != ("PERF", "3SM"):
                    continue
                target_candidates = records_by_signature.get(
                ...
                suggestion = target_record.word
                if target_tail:
                    suggestion = f"{suggestion} {target_tail}"
                suggestions = self._maltese_usage_mappings.setdefault(
                    source_surface,
                    [],
                )
                if suggestion not in suggestions:
        ...
        self._maltese_usage_verbs_finalized = True

    def _surface_maltese_usage_suggestion(
        self,
        suggestion: str,
        *,
        prefer_vowel_surface: bool,
    ) -> str:
        head, separator, tail = suggestion.partition(" ")
        surfaced_head = self._apply_initial_vowel_surface(
            head,
            head,
            prefer_vowel_surface=prefer_vowel_surface,
        )
        return f"{surfaced_head}{separator}{tail}" if separator else surfaced_head
    ...
    def _accepted_exact_english(self, text: str) -> bool:
        ...
        if " " in key:
            return (
                key in self.ACCEPTED_ENGLISH_PHRASES
                or key in self._english_usage_mappings
            )
        return (
            key in self.ACCEPTED_ENGLISH_WORDS
            or key in self._english_usage_mappings
        )
    ...
    def _accepted_article_english(self, word: str) -> bool:
        ...
        return bool(
            ...
            and " " not in key
            and (
                self._accepted_exact_english(key)
                or (
                    str(word).isupper()
                    and str(word).isalpha()
                    and 2 <= len(str(word)) <= 6
                )
            )
        )
    ...
    def _near_accepted_english_word(self, text: str) -> bool:
        ...
        accepted_words.update(
            word
            for word in self._english_usage_mappings
    ...
    def _english_token(
        ...
        *,
        ...
        suggestions = list(self._english_usage_mappings.get(key, ()))
        if not suggestions:
    ...
    def _maltese_usage_suggestions(self, word: str) -> list[str]:
        """Return preferred Maltese alternatives attached to accepted usage forms."""
        normalized = self._normalize_word(word)
        return list(self._maltese_usage_mappings.get(normalized, ()))
    ...
    def _exact_unquoted_english_phrase_at(
        ...
        for phrase in sorted(
            (
                value
                for value in (
                    *self.ACCEPTED_ENGLISH_PHRASES,
                    *self._english_usage_mappings,
                )
                if " " in value
            ),
    ...
    def _reset_request_token_cache(self) -> None:
        ...
        self._local.recognized_surface_cache = {}
        self._local.candidate_evidence_pool_cache = {}
        self._local.candidate_score_cache = {}
        self._local.suffix_surface_cache = {}
        self._local.suffix_exact_cache = {}
        self._local.suffix_correct_cache = {}
        self._local.suffix_suggest_cache = {}
        self._local.noun_possessive_correct_cache = {}
    ...
    def _request_recognized_surface_cache(self) -> dict[str, bool]:
        cache = getattr(self._local, "recognized_surface_cache", None)
        if cache is None:
            cache = {}
            self._local.recognized_surface_cache = cache
        return cache

    def _request_candidate_evidence_pool_cache(
        self,
    ) -> dict[tuple[str, tuple[str, ...]], CandidateEvidencePool]:
        cache = getattr(self._local, "candidate_evidence_pool_cache", None)
        if cache is None:
            cache = {}
            self._local.candidate_evidence_pool_cache = cache
        return cache

    def _request_candidate_score_cache(self) -> dict[tuple[str, str, str], ScoreRow]:
        cache = getattr(self._local, "candidate_score_cache", None)
        if cache is None:
            cache = {}
            self._local.candidate_score_cache = cache
        return cache

    def _request_suffix_surface_cache(self) -> dict[str, tuple]:
        cache = getattr(self._local, "suffix_surface_cache", None)
        if cache is None:
            cache = {}
            self._local.suffix_surface_cache = cache
        return cache

    def _request_suffix_exact_cache(self) -> dict[str, tuple]:
        cache = getattr(self._local, "suffix_exact_cache", None)
        if cache is None:
            cache = {}
            self._local.suffix_exact_cache = cache
        return cache

    def _request_suffix_correct_cache(self) -> dict[str, str | None]:
        cache = getattr(self._local, "suffix_correct_cache", None)
        if cache is None:
            cache = {}
            self._local.suffix_correct_cache = cache
        return cache

    def _request_noun_possessive_correct_cache(self) -> dict[str, str | None]:
        cache = getattr(self._local, "noun_possessive_correct_cache", None)
        if cache is None:
            cache = {}
            self._local.noun_possessive_correct_cache = cache
        return cache

    def _request_suffix_suggest_cache(self) -> dict[tuple[str, int], tuple[str, ...]]:
        cache = getattr(self._local, "suffix_suggest_cache", None)
        if cache is None:
            cache = {}
            self._local.suffix_suggest_cache = cache
        return cache
    ...
    def _allows_guttural_candidate(self, source: str, candidate: str) -> bool:
        ...
        # Do not insert għ into an existing consonant-i-consonant sequence.
        # This is a structural guard, not a lexical exception: jin may become
        # jien through i/ie, but it cannot become jgħin by inventing għ before
        # the already typed medial i.  aj/ej -> għi repairs remain available.
        source_g = self._graphemes(source_norm)
        candidate_g = self._graphemes(candidate_norm)
        if len(candidate_g) == len(source_g) + 1:
            for index in range(1, len(source_g) - 1):
                if (
                    source_g[index] == "i"
                    and source_g[index - 1] not in self.VOWELS
                    and source_g[index + 1] not in self.VOWELS
                    and candidate_g
                    == source_g[:index] + ["għ"] + source_g[index:]
                ):
                    return False
    ...
    def _negative_copula_surface_repair(self, word: str) -> str | None:
        """Restore the omitted h in short negative-copula spellings."""
        normalized = self._normalize_word(word)
        if normalized in {"mhiex", "mhix"}:
            return "mhix"
        if normalized == "mux":
            return "mhux"
        if normalized == "mu":
    ...
    def _ta_direct_object_gloss(self, word: str) -> str:
        normalized = self._normalize_word(word)
        gloss = self.TA_DIRECT_OBJECT_GLOSSES.get(normalized, "")
        if gloss:
            return gloss
        if normalized.endswith("x"):
            positive_gloss = self.TA_DIRECT_OBJECT_GLOSSES.get(normalized[:-1], "")
            if positive_gloss:
                return f"not {positive_gloss}"
        return ""
    ...
    def _phase_x_collect_candidates(self, word: str) -> TokenAnalysis:
        ...
        if self._ta_direct_object_gloss(normalized):
            ...
            priority_basic_candidate = normalized
            priority_basic_locked = True
        ...
        if orthographic is not None:
            final_aw_repair = orthographic.correct_final_aw_to_ghu(normalized)
            if final_aw_repair:
                add_unique(basic_candidates, final_aw_repair)
                if not priority_basic_locked:
                    priority_basic_candidate = final_aw_repair
                    priority_basic_locked = True
                    analysis.corrected = self._match_capitalisation(word, final_aw_repair)
                    analysis.is_deterministic = True

        gh_i_repairs = self._gh_i_to_e_repair_variants(normalized)
        if gh_i_repairs:
            add_unique(basic_candidates, gh_i_repairs[0])
            if not priority_basic_locked:
                priority_basic_candidate = gh_i_repairs[0]
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(word, gh_i_repairs[0])
                analysis.is_deterministic = True

        initial_e_match = self._validated_initial_e_match(normalized)
        if (
            initial_e_match
            and initial_e_match != normalized
            and self._verb_records_for_surface(initial_e_match)
        ):
            add_unique(basic_candidates, initial_e_match)
            if not priority_basic_locked:
                priority_basic_candidate = initial_e_match
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(word, initial_e_match)
                analysis.is_deterministic = True

        internal_vowel_variants = self._dictionary_internal_vowel_variants(normalized)
        if len(internal_vowel_variants) == 1:
            internal_vowel_repair = internal_vowel_variants[0]
            add_unique(basic_candidates, internal_vowel_repair)
            if not priority_basic_locked:
                priority_basic_candidate = internal_vowel_repair
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(
                    word,
                    internal_vowel_repair,
                )
                analysis.is_deterministic = True

        if (
            normalized in self.dictionary_set
            and bool(getattr(self._local, "bulk_mode", False))
            and not manual_repair
            and normalized != "tajru"
            and normalized not in self.EXACT_SUGGESTION_OVERRIDES
            and normalized not in self.MANUAL_WORD_SUGGESTIONS
            and self._missing_h_verb_repair(normalized) is None
            and self._initial_u_to_w_repair(normalized) is None
            and not self._has_strong_auto_repair_cue(normalized)
        ):
            analysis.basic_candidates = tuple(basic_candidates)
            analysis.complex_candidates = tuple(complex_candidates)
            analysis.x_candidates = tuple(basic_candidates or (normalized,))
            return analysis

        if normalized.startswith("l'") and len(normalized) > 2:
            tail = normalized[2:]
            if tail in {"hawn", "hemm", "hinn", "hawnhekk", "hemmhekk"}:
                add_unique(basic_candidates, normalized)
                priority_basic_candidate = normalized
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(word, normalized)
                analysis.is_deterministic = True
        if normalized in {"lejja", "lejk", "lejh", "lejha", "lejna", "lejkom", "lejhom", "sewja", "sewjk"}:
            add_unique(basic_candidates, normalized)
            priority_basic_candidate = normalized
            priority_basic_locked = True
            analysis.corrected = self._match_capitalisation(word, normalized)
        ...
        if normalized.startswith("l") and len(normalized) > 2:
            ...
            if (
                tail
                and tail[0] in self.VOWELS
                and self._is_probable_noun(tail)
                and not self._supports_l_apostrophe_tail(tail)
                and normalized not in {"lejja", "lejk", "lejh", "lejha", "lejna", "lejkom", "lejhom"}
            ):
        ...
        for local_prefix in ("l-", "il-", "'l-"):
            if normalized.startswith(local_prefix) and len(normalized) > len(local_prefix):
                local_tail = normalized[len(local_prefix) :]
                if local_tail in {"hawn", "hemm", "hinn", "hawnhekk", "hemmhekk"}:
                    local_surface = f"'l {local_tail}"
                    add_unique(complex_candidates, local_surface)
                    priority_basic_candidate = local_surface
                    priority_basic_locked = True
                    analysis.corrected = self._match_capitalisation(word, local_surface)
                    analysis.is_deterministic = True
                    break

        if normalized.startswith("mall-") and len(normalized) > 5:
            tail = normalized[5:]
            corrected_tail = self.correct_word(tail)
            corrected_tail_norm = self._normalize_word(corrected_tail)
            if corrected_tail_norm and self._is_probable_noun(corrected_tail_norm):
                corrected = f"mal-{corrected_tail_norm}"
                add_unique(complex_candidates, corrected)
                analysis.corrected = self._match_capitalisation(word, corrected)
                analysis.is_deterministic = True
        ...
        for metathesis_repair in self._guttural_vowel_metathesis_suffix_repairs(
            normalized
        ):
            add_unique(basic_candidates, metathesis_repair)
            if not priority_basic_locked:
                priority_basic_candidate = metathesis_repair
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(
                    word,
                    metathesis_repair,
        ...
        ido_l_surface = self._preferred_ido_l_surface(normalized)
        if ido_l_surface is not None:
            add_unique(basic_candidates, ido_l_surface)
            if not priority_basic_locked:
                priority_basic_candidate = ido_l_surface
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(
                    word,
                    ido_l_surface,
        ...
        for glide_repair in self._pronominal_suffix_glide_variants(normalized):
            add_unique(basic_candidates, glide_repair)
            if not priority_basic_locked:
                priority_basic_candidate = glide_repair
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(word, glide_repair)
                analysis.is_deterministic = True
            break

        missing_pronominal_h = self._missing_pronominal_h_variants(normalized)
        if missing_pronominal_h:
            add_unique(basic_candidates, missing_pronominal_h[0])
            if not priority_basic_locked:
                priority_basic_candidate = missing_pronominal_h[0]
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(
                    word,
                    missing_pronominal_h[0],
                )
                analysis.is_deterministic = True
        ...
        direct_object_guttural_repairs = self._restore_direct_object_guttural_suffix(
            normalized
        )
        if direct_object_guttural_repairs:
            add_unique(basic_candidates, direct_object_guttural_repairs[0])
            analysis.corrected = self._match_capitalisation(
                word,
                direct_object_guttural_repairs[0],
            )
            analysis.is_deterministic = True

        hollow_ju_repair = self._hollow_ju_suffix_vowel_repair(normalized)
        if hollow_ju_repair is not None:
            add_unique(basic_candidates, hollow_ju_repair)
            analysis.corrected = self._match_capitalisation(word, hollow_ju_repair)
            analysis.is_deterministic = True
        ...
        if normalized.startswith("kemmil"):
            compact_kemm_il = "kemm-il" + normalized[len("kemmil") :]
            if self._is_recognized_surface(compact_kemm_il):
                add_unique(basic_candidates, compact_kemm_il)
                priority_basic_candidate = compact_kemm_il
                analysis.corrected = self._match_capitalisation(
                    word,
                    compact_kemm_il,
                )
        ...
        if (
            ...
            for seed in stem_seeds:
                ...
                for candidate_seed in candidate_seeds:
                    recovered = self._correct_suffix_cached(candidate_seed)
                    for candidate in (candidate_seed, recovered):
                        if candidate and self._is_verb_tagged_word(candidate):
                            ...
                            if (
                                ...
                                and priority_basic_candidate is None
                                and self._normalize_word(surface_candidate) != normalized
                            ):
                                priority_basic_candidate = surface_candidate
                                analysis.corrected = self._match_capitalisation(
                                    word,
                                    surface_candidate,
                                )
        ...
        if normalized.startswith("i") and len(normalized) >= 5:
            base_without_i = normalized[1:]
            doubled_generator = getattr(self, "doubled_letter_generator", None)
            if doubled_generator is not None:
                for doubled_base in doubled_generator.missing_double_variants(base_without_i):
                    doubled_base = self._normalize_word(doubled_base)
                    if (
                        doubled_base
                        and doubled_base != base_without_i
                        and self._is_recognized_surface(doubled_base)
                        and (
                            any(
                                tag.startswith("PASTPAR")
                                or tag.split("-", 1)[0].startswith("ADJ")
                                for tag in self.word_tags.get(doubled_base, ())
                            )
                            or self._verb_uses_initial_i_prefix(doubled_base)
                        )
                    ):
                        add_unique(basic_candidates, doubled_base)
                        if not priority_basic_locked:
                            priority_basic_candidate = doubled_base
                            analysis.corrected = self._match_capitalisation(
                                word,
                                doubled_base,
                            )
        ...
        if possessive_surface is not None:
            ...
            if possessive_surface != normalized:
                priority_basic_candidate = possessive_surface
                priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(
                    word,
                    possessive_surface,
        ...
        if lexicalized_forms:
            ...
            if lexicalized_forms[0] != normalized and not priority_basic_locked:
                analysis.corrected = self._match_capitalisation(
                    word,
                    lexicalized_forms[0],
                )
        ...
        if terminal_apostrophe is not None:
            ...
            if not priority_basic_locked:
                priority_basic_candidate = terminal_apostrophe
        ...
        if fixed_time is None and self._has_basic_repair_cue(normalized):
            ...
            if orthographic is not None and "gh" in normalized and hasattr(
                orthographic,
                "correct_gh_priority",
            ):
                gh_priority = orthographic.correct_gh_priority(normalized)
                if gh_priority in verified_basic_repairs:
                    verified_basic_repairs = [
                        gh_priority,
                        *[
                            candidate
                            for candidate in verified_basic_repairs
                            if candidate != gh_priority
                        ],
                    ]
            terminal_apostrophe_priority = self._strict_terminal_apostrophe_match(
                normalized
            )
            if terminal_apostrophe_priority in verified_basic_repairs:
                verified_basic_repairs = [
                    terminal_apostrophe_priority,
                    *[
                        candidate
                        for candidate in verified_basic_repairs
                        if candidate != terminal_apostrophe_priority
                    ],
                ]
            verified_basic_repairs = [
                candidate
                for _, candidate in sorted(
                    enumerate(verified_basic_repairs),
                    key=lambda item: (
                        0 if self._noun_possessive_base_for_surface(item[1]) else 1,
                        item[0],
                    ),
                )
            ]
            ...
            if verified_basic_repairs:
                non_identity_repairs = [
                    candidate
                    for candidate in verified_basic_repairs
                    if self._normalize_word(candidate) != normalized
                ]
                current_norm = self._normalize_word(analysis.corrected)
                first_repair = self._normalize_word(
                    non_identity_repairs[0] if non_identity_repairs else verified_basic_repairs[0]
                )
        ...
        for candidate in self._strict_lookup_variants(normalized):
            ...
            if (
                ...
                and normalized not in self.dictionary_set
                and (
            ...
            collapsed = self._collapse_invalid_glide_doubling(candidate)
            if collapsed and self._is_recognized_surface(collapsed):
                add_unique(basic_candidates, collapsed)
                if not priority_basic_locked:
                    priority_basic_candidate = collapsed
        final_accent = self._dictionary_final_vowel_accent(normalized)
        if final_accent:
            add_unique(basic_candidates, final_accent)
            if not priority_basic_locked:
                priority_basic_candidate = final_accent
        ...
        orthographic = getattr(self, "orthographic_generator", None)
        suffix_repair_candidates = self._suffix_repair_variants(normalized)
        for candidate in suffix_repair_candidates:
            add_unique(basic_candidates, candidate)
            # Compose a suffix spelling repair with de-gemination and removal
            # of a duplicated epenthetic i. Keep only exact generated verb
            # surfaces so this cannot become a broad vowel-deletion rule.
            if orthographic is None:
                continue
            for reduced in orthographic.remove_extra_double_variants(candidate):
                structural_candidates = [reduced]
                reduced_graphemes = self._graphemes(reduced)
                if (
                    len(reduced_graphemes) >= 4
                    and reduced_graphemes[0] == "i"
                    and reduced_graphemes[1] not in self.VOWELS
                    and reduced_graphemes[2] == "i"
                    and reduced_graphemes[3] not in self.VOWELS
                ):
                    structural_candidates.append(
                        self._from_graphemes(
                            reduced_graphemes[:2] + reduced_graphemes[3:]
                        )
                    )
                for structural in structural_candidates:
                    exact_generated = bool(
                        self._exact_suffix_matches_cached(structural)
                    )
                    if (
                        not exact_generated
                        and structural == reduced
                        and structural.startswith("i")
                    ):
                        exact_generated = any(
                            self._exact_suffix_matches_cached(surface)
                            for surface in self._initial_i_variants(structural)
                        )
                    if not exact_generated:
                        continue
                    add_unique(basic_candidates, structural)
                    if not priority_basic_locked:
                        priority_basic_candidate = structural
                        priority_basic_locked = True
                        analysis.corrected = self._match_capitalisation(
                            word,
                            structural,
            ...
            for candidate in orthographic.substitute_i_ie(normalized):
                if (
                    ...
                    if (
                        ...
                        and (
                            (
                                candidate.endswith("ie")
                                and self._is_verb_tagged_word(candidate)
                            )
                            or priority_basic_candidate is None
                            or self._word_distance(normalized, candidate)
                            < self._word_distance(normalized, priority_basic_candidate)
                        )
                    ):
                        priority_basic_candidate = candidate
                    break
        for candidate in self._missing_medial_guttural_variants(normalized):
            add_unique(basic_candidates, candidate)
            if (
                not priority_basic_locked
                and normalized not in self.dictionary_set
        ...
        if suffix_generator is not None:
            suffix_surface = self._correct_suffix_cached(normalized)
            if (
                suffix_surface is not None
                and self._suffix_anchor_compatible(normalized, suffix_surface)
            ):
                if suffix_surface != normalized and normalized.endswith("h"):
                    for suffix_candidate in self._valid_suffix_surface_candidates(
                        suffix_surface
                    ):
                        if "-F8-" not in suffix_candidate.raw_tag:
                            continue
                        for index, token in enumerate(self._graphemes(suffix_surface)):
                            if token not in self.VOWELS:
                                continue
                            compact_surface = self._from_graphemes(
                                self._graphemes(suffix_surface)[:index]
                                + self._graphemes(suffix_surface)[index + 1 :]
                            )
                            if compact_surface == normalized:
                                add_unique(basic_candidates, normalized)
                                priority_basic_candidate = normalized
                                priority_basic_locked = True
                                analysis.corrected = self._match_capitalisation(
                ...
                if (
                    suffix_surface != normalized
                    and priority_basic_candidate is None
                    and normalized not in self.dictionary_set
                    and not self._is_recognized_surface(normalized)
                ):
        ...
        # Pronominal suffix gliding repair: titfijulix -> titfihulix.
        # An erroneous glide letter (j/w) in the pronominal suffix position is
        # detected and replaced with the expected vowel/h surface.
        if (
            normalized not in self.dictionary_set
            and not self._is_recognized_surface(normalized)
        ):
            gliding_repair = self._pronominal_suffix_gliding_repair(normalized)
            if gliding_repair is not None and gliding_repair != normalized:
                add_unique(basic_candidates, gliding_repair)
                if not priority_basic_locked:
                    priority_basic_candidate = gliding_repair
                    priority_basic_locked = True
                analysis.corrected = self._match_capitalisation(word, gliding_repair)
                analysis.is_deterministic = True

        # Fused preposition repair: fwicci -> f'wiċċi, bwicci -> b'wiċċi, etc.
        # This is the same deterministic match used by _correct_word_uncached but
        # wired into Phase X so correct_word() also benefits from it.
        if not priority_basic_locked and normalized not in self.dictionary_set:
            ...
            if fused_preposition_rules is not None:
                ...
                if fused_match is not None:
                    fused_corrected = fused_match.corrected
                    add_unique(complex_candidates, fused_corrected)
                    analysis.corrected = self._match_capitalisation(word, fused_corrected)
                    analysis.is_deterministic = True
                    priority_basic_locked = True
        ...
        if (
            ...
            seeds = [normalized]
            seeds.extend(self._initial_i_variants(normalized))
            seeds.extend(
                orthographic.shortcut_letter_variants(
                    normalized,
                    max_changes=1,
            ...
            for seed in seeds:
                ...
                for doubled_seed in doubled_generator.missing_double_variants(seed):
                    if is_exact_y_surface(doubled_seed):
                        ...
                        if (
                            doubled_seed != normalized
                            and not priority_basic_locked
                            and normalized not in self.dictionary_set
                            and priority_basic_candidate is None
                        ):
                            priority_basic_candidate = doubled_seed
                ...
                for h_seed in orthographic.insert_token_next_to_vowels(seed, "h"):
                    if is_exact_y_surface(h_seed):
                        ...
                        if (
                            ...
                            priority_basic_candidate = h_seed
        ...
        if attached_l is not None:
            ...
            add_unique(basic_candidates, attached_l)
            priority_basic_candidate = attached_l
            priority_basic_locked = True
            analysis.corrected = self._match_capitalisation(word, attached_l)
            analysis.is_deterministic = True
        ...
        if initial_u_w is not None:
            ...
            if not priority_basic_locked:
                priority_basic_candidate = initial_u_w
        ...
        if pattern_priority is not None and not priority_basic_locked:
            analysis.corrected = self._match_capitalisation(word, pattern_priority)
            analysis.is_deterministic = True
            priority_basic_candidate = None
            priority_basic_locked = True
        ...
        if (
            ...
            for candidate in basic_candidates:
                if candidate.endswith("ie") and self._is_verb_tagged_word(candidate):
                    priority_basic_candidate = candidate
                    break
        if not priority_basic_locked and normalized not in self.dictionary_set:
            for candidate in basic_candidates:
                candidate_norm = self._normalize_word(candidate)
                if (
                    self._is_verb_tagged_word(candidate_norm)
                    and "ie" in candidate_norm
                    and "ie" not in normalized
                    and candidate_norm.replace("ie", "i", 1) in {
                        normalized,
                        normalized.replace("g", "ġ", 1),
                    }
                ):
                    priority_basic_candidate = candidate_norm
                    break
        ...
        if normalized not in self.dictionary_set:
            possessive_repairs = [
                self._normalize_word(candidate)
                for candidate in basic_candidates
                if (
                    self._normalize_word(candidate) != normalized
                    and self._noun_possessive_base_for_surface(
                        self._normalize_word(candidate)
                    )
                )
            ]
            if possessive_repairs:
                current_corrected_norm = self._normalize_word(analysis.corrected)
                if (
                    not current_corrected_norm
                    or current_corrected_norm not in self.dictionary_set
                ):
                    analysis.corrected = self._match_capitalisation(
                        word,
                        possessive_repairs[0],
                    )
                    analysis.is_deterministic = True
            dictionary_repairs = [
                self._normalize_word(candidate)
                ...
                if (
                    self._normalize_word(candidate) != normalized
                    and self._normalize_word(candidate) in self.dictionary_set
                )
            ]
            preferred_number_repairs = self.NUMBER_FORM_REPAIRS.get(normalized, ())
            if dictionary_repairs and preferred_number_repairs:
                repair_rank = {
                    self._normalize_word(candidate): rank
                    for rank, candidate in enumerate(preferred_number_repairs)
                }
                dictionary_repairs.sort(
                    key=lambda candidate: repair_rank.get(candidate, len(repair_rank))
                )
            if dictionary_repairs:
                current_corrected_norm = self._normalize_word(analysis.corrected)
                if (
                    not current_corrected_norm
                    or (
                        current_corrected_norm not in self.dictionary_set
                        and not self._noun_possessive_base_for_surface(
                            current_corrected_norm
                        )
                    )
                ):
                    analysis.corrected = self._match_capitalisation(
                        word,
                        dictionary_repairs[0],
                    )
                    analysis.is_deterministic = True
            for candidate in basic_candidates:
                candidate_norm = self._normalize_word(candidate)
                if (
                    not analysis.is_deterministic
                    and
                    candidate_norm.startswith("tiegħ")
                    and normalized.startswith("tij")
                    and candidate_norm in self.dictionary_set
                ):
                    analysis.corrected = self._match_capitalisation(word, candidate_norm)
                    analysis.is_deterministic = True
                    break
                if (
                    self._is_verb_tagged_word(candidate_norm)
        ...
        # A recursive helper may populate the request-local TokenAnalysis while
        # Phase X is still collecting evidence.  Never let that side effect
        # select a surface which this pass rejected or did not generate.
        admitted = {
            self._normalize_word(candidate)
            for candidate in (*basic_candidates, *complex_candidates)
            if candidate
        }
        current_corrected = self._normalize_word(analysis.corrected)
        if current_corrected and current_corrected not in admitted:
            fallback = next(
                (
                    candidate
                    for candidate in basic_candidates
            ...
            analysis.corrected = self._match_capitalisation(word, fallback)
            analysis.is_deterministic = fallback != normalized
    ...
    def _vowel_digraph_repair_variants(self, word: str) -> list[str]:
        ...
        def add(candidate: str, *, allow_exact_alt: bool = False) -> None:
            candidate = self._normalize_word(candidate)
    ...
    def _has_basic_repair_cue(self, word: str) -> bool:
        ...
        return bool(
            ...
            or bool(
                getattr(self, "orthographic_generator", None)
                and self.orthographic_generator.substitute_stop_pairs_in_cluster(
                    normalized
                )
            )
    ...
    def _has_strong_auto_repair_cue(self, word: str) -> bool:
        """Whether an unknown surface has enough evidence for auto-rewrite.

        A plain unknown word can still be a valid missing dictionary entry.
        Keep automatic correction for visible Maltese spelling cues, but avoid
        promoting distant generated suffix/adverbial forms from ordinary-looking
        words such as participles or nouns.
        """
        normalized = self._normalize_word(word)
        if not normalized:
            return False
        ...
        return bool(
            "gh" in normalized
            or any(letter in normalized for letter in ("h", "c", "z", "g", "q"))
            or any(sequence in normalized for sequence in ("eja", "ea", "ey", "ay", "io"))
            or any(
                left == mid == right and left not in self.VOWELS
                for left, mid, right in zip(graphemes, graphemes[1:], graphemes[2:])
            )
        )
    ...
    def _dictionary_verified_basic_repairs(self, word: str) -> list[str]:
        ...
        def is_valid(candidate: str) -> bool:
            verb_index = getattr(getattr(self, "suffix_generator", None), "verb_index", None)
            return bool(
                candidate in self.dictionary_set
                or any(tag.startswith(("T-", "Q-", "S-", "AS-", "IS-", "VERB")) for tag in self.word_tags.get(candidate, set()))
                or (verb_index is not None and verb_index.word_records(candidate))
        ...
        if orthographic is not None:
            ...
            for candidate in orthographic.substitute_i_ie(normalized):
                add_seed(candidate)
            for candidate in orthographic.substitute_g_k_in_cluster(normalized):
                add_seed(candidate)
            for candidate in orthographic.substitute_stop_pairs_in_cluster(normalized):
                add_seed(candidate)
            for candidate in orthographic.final_aw_to_ghu_variants(normalized):
                add_seed(candidate)
            ...
            for seed in tuple(seeds):
                for candidate in orthographic.shortcut_letter_variants(
                    ...
                    add_seed(candidate)
                for candidate in self._dictionary_internal_vowel_variants(seed):
                    add_seed(candidate)
                ...
                for candidate in self._collapse_vowel_around_token_variants(seed, "għ"):
                    add_seed(candidate)
                for candidate in self._gh_i_to_e_repair_variants(seed):
                    add_seed(candidate)
        ...
        double_groups = sum(
            ...
            for index, (left, right) in enumerate(
                zip(self._graphemes(normalized), self._graphemes(normalized)[1:])
            )
            if (
                left == right
                and left not in self.VOWELS
                and (
                    index == 0
                    or self._graphemes(normalized)[index - 1] != left
                )
            )
        )
        if (
            accepted
            and double_groups < 2
            and not any(sequence in normalized for sequence in ("ey", "ay"))
        ):
        ...
        for seed in tuple(seeds):
            ...
            if orthographic is not None:
                follow_ups.extend(
                    orthographic.remove_extra_double_variants(seed)
                )
    ...
    def _gh_i_to_e_repair_variants(self, word: str) -> list[str]:
        """Return dictionary-gated għiC -> għeC spelling variants."""
        normalized = self._normalize_word(word).replace("ghi", "għi")
        graphemes = self._graphemes(normalized)
        variants: list[str] = []
        for index in range(len(graphemes) - 2):
            if (
                graphemes[index] == "għ"
                and graphemes[index + 1] == "i"
                and graphemes[index + 2] not in self.VOWELS
            ):
    ...
    def _is_direct_dictionary_shortcut(
        self,
        original_word: str,
        candidate: str,
    ) -> bool:
        ...
        if orthographic is None:
            return False
        candidate_norm = self._normalize_word(candidate)
        return candidate_norm in {
            self._normalize_word(variant)
            for variant in orthographic.dictionary_shortcut_variants(original_word)
        }
    ...
    def _direct_object_accent_surface(self, word: str) -> str | None:
        ...
        if suffix_generator is None:
            return None
        candidate = self._correct_suffix_cached(normalized)
    ...
    def _phase_y_basic_resolution(
        ...
        if valid_basic_candidates:
            if (
                normalized in analysis.basic_candidates
                and not self._has_strong_auto_repair_cue(normalized)
            ):
                return None
    ...
    def _phase_w_seed_suggestions(
        ...
        *,
        ...
        def add(candidate: str, *, allow_exact_alt: bool = False) -> None:
            candidate = self._normalize_word(candidate)
            corrected_norm = self._normalize_word(analysis.corrected or "")
            if (
                candidate == source_norm
                and corrected_norm
                and corrected_norm != source_norm
                and source_norm not in self.dictionary_set
                and not self._is_recognized_surface(source_norm)
                and not self._exact_suffix_matches_cached(source_norm)
            ):
                return
            if (
            ...
            if (
                ...
                and candidate != source_norm
                and not self._has_strong_auto_repair_cue(source_norm)
            ):
                return
            if (
                source_norm in self.dictionary_set
        ...
        add(
            analysis.corrected or analysis.normalized,
            allow_exact_alt=bool(
                analysis.corrected
                and self._normalize_word(analysis.corrected) != source_norm
            ),
        )
        ...
        for candidate in self._lexicalized_form_variants(analysis.normalized):
            add(candidate)
        for candidate in self._short_missing_guttural_vowel_variants(analysis.normalized):
            add(candidate)
        for candidate in self._medial_guttural_vowel_restoration_variants(
            analysis.normalized
        ):
            add(candidate)
        for candidate in self._missing_h_before_r_verb_variants(analysis.normalized):
            add(candidate, allow_exact_alt=True)
        for candidate in self._pattern_repair_variants(analysis.normalized):
            if candidate in self.dictionary_set or self._is_verb_tagged_word(candidate):
                add(candidate, allow_exact_alt=True)
        if len(seeded) < limit:
    ...
    def _z_ma_phrase_decision(
        ...
        *,
        ...
        corrected_next: str,
        is_verb: bool,
    ) -> FunctionPhraseDecision | None:
        corrected_next_norm = self._normalize_word(corrected_next)
        bare_ma_negatives = {"tantx"}
        article_tail = ""
        if corrected_next_norm.startswith("il-"):
            article_tail = corrected_next_norm[3:]
        elif corrected_next_norm.startswith(("l-", "l'")):
            article_tail = corrected_next_norm[2:]
        if article_tail and not is_verb:
            corrected_phrase = self._match_capitalisation(
                original_word,
                f"mal-{article_tail}",
            )
            return FunctionPhraseDecision(
                corrected_phrase=corrected_phrase,
                token_original=original_word,
                token_corrected=corrected_phrase,
                choices=[
                    {
                        "word": corrected_phrase,
                        "meaning": self.meaning_for(article_tail),
                    }
                ],
                previous_surface=self._normalize_word(corrected_phrase),
                force_crucial=True,
            )
        contracts_to_m = (
            bool(corrected_next_norm)
            and not is_verb
            and (
                corrected_next_norm.startswith(("għ", "gh", "h"))
                or corrected_next_norm[0] in self.VOWELS
            )
        )
        ...
        needs_contract = (
            is_verb
            and self._contract_negative_ma(f"ma {corrected_next}")
            != f"ma {corrected_next}"
        )
        if corrected_next_norm in bare_ma_negatives:
            display_ma = self._match_capitalisation(original_word, "ma")
            corrected_phrase = f"{display_ma} {corrected_next}"
            return FunctionPhraseDecision(
                corrected_phrase=corrected_phrase,
                token_original=f"{original_word} {corrected_next}",
                token_corrected=corrected_phrase,
                choices=[],
                previous_surface=corrected_next_norm,
                force_crucial=False,
            )
        if (
            current_norm == target_ma
            and corrected_next_norm == next_norm
            and not needs_contract
        ):
            return None

        display_ma = self._match_capitalisation(original_word, target_ma)
        corrected_phrase = f"{display_ma} {corrected_next}"
        if is_verb:
            corrected_phrase = self._contract_negative_ma(corrected_phrase)
        elif target_ma == "m'":
            corrected_phrase = self._match_capitalisation(
                original_word,
                f"m'{corrected_next_norm}",
            )
        ...
        if current_norm != target_ma and is_verb:
            display_other = self._match_capitalisation(original_word, "ma'")
            choices.append(
                {
        ...
        return FunctionPhraseDecision(
            ...
            token_corrected=corrected_phrase,
            choices=choices,
            previous_surface=self._normalize_word(corrected_phrase.split()[-1]),
            force_crucial=current_norm != target_ma,
        )

    def _z_min_phrase_decision(
        self,
        *,
        original_word: str,
        current_norm: str,
        ...
        corrected_next_norm = self._normalize_word(corrected_next)
        interrogative_followers = {"kien", "hu", "hi", "huma", "xiex", "min"}
        source_followers = {
            "fejn", "hemm", "hawn", "hinn", "hemmhekk", "hawnhekk",
            "hekk", "fuq", "naħa", "naha", "malta",
        }
        if next_norm == "aw":
            target_min = "min"
        elif corrected_next_norm in interrogative_followers:
            target_min = "min"
        elif (
            self._is_verb_tagged_word(corrected_next_norm)
            or corrected_next_norm.startswith(("j", "i", "t", "n", "jgħ", "m", "b"))
            or corrected_next_norm in {"iwerżaq", "iwerzaq", "jitlob", "jgħid", "jghid", "iħares", "ihares", "iparla", "parla"}
        ) and not (corrected_next_norm in source_followers or self._exact_place_word(corrected_next)):
            target_min = "min"
        elif corrected_next_norm in source_followers or self._exact_place_word(corrected_next):
            target_min = "minn"
        else:
            target_min = current_norm
        if current_norm == target_min and corrected_next_norm == next_norm:
            if target_min not in {"min", "minn"}:
                return None
        ...
        if (
            current_norm != target_min
            or next_norm == "aw"
            or corrected_next_norm != next_norm
        ) and corrected_next_norm not in interrogative_followers | {"fejn"}:
        ...
        return FunctionPhraseDecision(
            corrected_phrase=corrected_phrase,
            token_original=original_word,
            token_corrected=display_min,
            choices=choices,
            previous_surface=self._normalize_word(corrected_phrase.split()[-1]),
            force_crucial=current_norm != target_min,
        )

    def _z_ta_phrase_decision(
        self,
        *,
        original_word: str,
        corrected_next: str,
        ...
        next_markers = self._word_tag_markers(corrected_next_norm)
        is_bare_adverb = (
            (
                "ADVERB" in next_markers
                and "PREP" not in next_markers
                and "DET" not in next_markers
            )
            or (
                corrected_next_norm.startswith("i")
                and self._is_adverb_tagged_word(corrected_next_norm[1:])
            )
        )
        if is_bare_adverb:
            corrected_phrase = f"{ta_surface}' {corrected_next}"
        elif (
            self._starts_vowel_gh_or_h(corrected_next)
            and corrected_next_norm
            and corrected_next_norm[0] in self.VOWELS
        ):
        ...
        allow_bare_ta_choice = (
            corrected_next_norm == "kemm"
            or is_bare_adverb
        )
        meaning_word = (
            corrected_next_norm[1:]
            if corrected_next_norm.startswith("i")
            and self._is_adverb_tagged_word(corrected_next_norm[1:])
            else corrected_next_norm
        )
        next_meaning = self._get_meaning_index().meaning_for(meaning_word)
        choices = [
            {
                "word": corrected_phrase,
                "meaning": f"of {next_meaning}" if next_meaning else "of",
                "suggestion_kind": "function_word_context",
            }
        ]
        if allow_bare_ta_choice:
            choices.append(
                {
                    "word": alternate_phrase,
                    "meaning": (
                        f"he gave {next_meaning}" if next_meaning else "he gave"
                    ),
                    "suggestion_kind": "function_word_context",
                }
            )
        ...
        *,
        ...
        prefer_vowel_surface = (
            ...
            and (
                ...
                or self._initial_i_participial_adjective_surface(corrected_word)
                or vowel_options
                or plain_options
            )
    ...
    def _store_correct_word_result(
        ...
        *,
        ...
        if normalized_orig and normalized_corr and " " not in normalized_corr:
            ...
            elif normalized_orig.startswith("i"):
                participial_i = self._initial_i_participial_adjective_surface(normalized_corr)
                if participial_i is not None:
                    corrected = self._match_capitalisation(word, participial_i)
    ...
    def _valid_suffix_surface_candidates(self, word: str):
        ...
        normalized = self._normalize_word(word)
        cache = self._request_suffix_surface_cache()
        cached = cache.get(normalized)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["suffix_surface"] = (
                    profiler.cache_hits.get("suffix_surface", 0) + 1
                )
            return list(cached)

        candidates = suffix_generator.candidates_for_surface(normalized)
        if not candidates:
            cache[normalized] = tuple()
            if profiler:
                profiler.cache_misses["suffix_surface"] = (
                    profiler.cache_misses.get("suffix_surface", 0) + 1
                )
            return []

        alternative_do_2s_bases = {
            self._normalize_word(getattr(candidate, "base", ""))
            for candidate in candidates
            if getattr(candidate, "suffix_kind", "") == "DO"
            and getattr(candidate, "suffix_person", "") == "2S"
            and getattr(candidate, "rule_id", "") != "DEFAULT_ADD"
        }

        def is_invalid_default_do_2s(candidate) -> bool:
            if (
                getattr(candidate, "suffix_kind", "") != "DO"
                or getattr(candidate, "suffix_person", "") != "2S"
                or getattr(candidate, "rule_id", "") != "DEFAULT_ADD"
            ):
                return False
            base = self._normalize_word(getattr(candidate, "base", ""))
            surface = self._normalize_word(getattr(candidate, "surface", ""))
            return bool(
                base
                and base in alternative_do_2s_bases
                and base[-1] not in self.VOWELS
                and surface in {f"{base}ek", f"{base}ok"}
            )

        def is_invalid_intransitive_direct_object(candidate) -> bool:
            suffix_kind = getattr(candidate, "suffix_kind", "")
            if suffix_kind not in {"DO", "DO_IDO"}:
                return False
            raw_tag = str(getattr(candidate, "raw_tag", "") or "").lower()
            return "-to be " in raw_tag or "-to not be " in raw_tag

        result = tuple(
            candidate
            for candidate in candidates
            if not is_invalid_default_do_2s(candidate)
            and not is_invalid_intransitive_direct_object(candidate)
            and not is_invalid_imperative_suffix_combination(
    ...
    def _exact_suffix_matches_cached(self, word: str):
        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is None:
            return []
        normalized = self._normalize_word(word)
        cache = self._request_suffix_exact_cache()
        cached = cache.get(normalized)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["suffix_exact"] = (
                    profiler.cache_hits.get("suffix_exact", 0) + 1
                )
            return list(cached)
        result = tuple(suffix_generator.exact_suffix_matches(normalized))
        cache[normalized] = result
        if profiler:
            profiler.cache_misses["suffix_exact"] = (
                profiler.cache_misses.get("suffix_exact", 0) + 1
            )
        return list(result)

    def _should_try_suffix_correction(self, word: str) -> bool:
        normalized = self._normalize_word(word)
        suffix_generator = getattr(self, "suffix_generator", None)
        if (
            not normalized
            or suffix_generator is None
            or not hasattr(suffix_generator, "might_have_suffix")
            or not suffix_generator.might_have_suffix(normalized)
        ):
            return False
        if normalized in self.dictionary_set and not self._has_strong_auto_repair_cue(
            normalized
        ):
            return False
        if self._has_strong_auto_repair_cue(normalized):
            return True
        if normalized.startswith(("n", "t", "j", "in", "it", "ji", "ni", "ti")):
            return True
        return False

    def _correct_suffix_cached(self, word: str) -> str | None:
        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is None:
            return None
        normalized = self._normalize_word(word)
        if not self._should_try_suffix_correction(normalized):
            return None
        cache = self._request_suffix_correct_cache()
        if normalized in cache:
            profiler = current_profiler()
            if profiler:
                profiler.cache_hits["suffix_correct"] = (
                    profiler.cache_hits.get("suffix_correct", 0) + 1
                )
            return cache[normalized]
        result = suffix_generator.correct_suffix(normalized)
        cache[normalized] = result
        profiler = current_profiler()
        if profiler:
            profiler.cache_misses["suffix_correct"] = (
                profiler.cache_misses.get("suffix_correct", 0) + 1
            )
        return result

    def _suggest_suffixes_cached(self, word: str, *, limit: int) -> list[str]:
        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is None or not hasattr(suffix_generator, "suggest_suffixes"):
            return []
        normalized = self._normalize_word(word)
        if not self._should_try_suffix_correction(normalized):
            return []
        cache = self._request_suffix_suggest_cache()
        cache_key = (normalized, limit)
        cached = cache.get(cache_key)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["suffix_suggest"] = (
                    profiler.cache_hits.get("suffix_suggest", 0) + 1
                )
            return list(cached)
        result = tuple(suffix_generator.suggest_suffixes(normalized, limit=limit))
        cache[cache_key] = result
        if profiler:
            profiler.cache_misses["suffix_suggest"] = (
                profiler.cache_misses.get("suffix_suggest", 0) + 1
            )
        return list(result)
    ...
    def _is_verb_tagged_word(self, word: str) -> bool:
        ...
        if (
            ...
            return False
        return bool(self._exact_suffix_matches_cached(normalized))
    ...
    def _verb_records_for_surface(self, word: str):
        ...
        generated = self._exact_suffix_matches_cached(normalized)
        return [
            record
    ...
    def _negative_verb_form(self, word: str) -> str | None:
        """Return the matching negative verb surface when the paradigm has one."""
        normalized = self._normalize_word(word)
        if not normalized or normalized.endswith("x"):
            return normalized if self._is_verb_tagged_word(normalized) else None

        if self._valid_suffix_surface_candidates(normalized):
            # Productive DO/IDO suffixes stay attached in the negative form;
            # the negative marker follows the complete generated surface.
            return f"{normalized}x"

        records = self._verb_records_for_surface(normalized)
        suffix_generator = getattr(self, "suffix_generator", None)
        verb_index = getattr(suffix_generator, "verb_index", None)
        if not records or verb_index is None:
            return None
        ...
        for record in records:
            for related in verb_index.by_short_tag.get(record.short_tag, ()):
                if (
                    related.tense == record.tense
                    and related.person == record.person
                    and related.word.endswith("x")
                ):
                    matches.add(related.word)

        if not matches:
            direct = f"{normalized}x"
            return direct if self._is_verb_tagged_word(direct) else None
        return min(
            matches,
            key=lambda candidate: (
    ...
    def _initial_i_surface_repair(self, word: str) -> str | None:
        ...
        # The negative marker belongs to the complete verb surface. Apply the
        # same i-/non-i decision to its positive base, then restore -x.
        if normalized.endswith("x") and len(normalized) > 2:
            repaired_base = self._initial_i_surface_repair(normalized[:-1])
            if repaired_base and repaired_base != normalized[:-1]:
                return repaired_base + "x"
    ...
    def _preposition_pronoun_suffix_base(self, word: str) -> str | None:
        """Recognise preposition/adverb + pronoun surfaces like quddiemhom."""
        normalized = self._normalize_word(word)
        suffixes = ("kom", "hom", "ek", "ha", "na", "i", "u")
        for suffix in suffixes:
            if not normalized.endswith(suffix) or len(normalized) <= len(suffix) + 2:
                continue
            base = normalized[: -len(suffix)]
            if self._is_preposition_tagged_word(base) or self._is_adverb_tagged_word(base):
                return base
        return None

    def _coarse_pos_classes(self, word: str) -> frozenset[str]:
        normalized = self._normalize_word(word)
        classes: set[str] = set()
        if self._is_noun_tagged_word(normalized):
            classes.add("noun")
        if self._is_adjective_tagged_word(normalized):
            classes.add("adjective")
        if self._is_verb_tagged_word(normalized):
            classes.add("verb")
        if self._is_pronoun_tagged_word(normalized):
            classes.add("pronoun")
        if self._is_adverb_tagged_word(normalized):
            classes.add("adverb")
        if self._is_preposition_tagged_word(normalized):
            classes.add("preposition")
        return frozenset(classes)

    def _suggestion_pos_compatible(self, corrected: str, candidate: str) -> bool:
        corrected_pos = self._coarse_pos_classes(corrected)
        candidate_pos = self._coarse_pos_classes(candidate)
        if not corrected_pos or not candidate_pos:
            return True
        if corrected_pos & candidate_pos:
            return True
        return bool(
            corrected_pos <= {"noun", "adjective"}
            and candidate_pos <= {"noun", "adjective"}
        )
    ...
    def _is_li_relative_adjective_allowed(self, word: str) -> bool:
        ...
        if article_rules is None:
            return False
        adjective_meaning = self._get_meaning_index().meaning_for(normalized)
        if not adjective_meaning:
            adjective_meaning = " / ".join(self._tag_meanings_for_word(normalized))
        return bool(
            article_rules._superlative_meaning(adjective_meaning)
        )
    ...
    def _safe_contracted_fb_tail(self, tail: str) -> str | None:
        ...
        if (
            ...
            or bool(self._get_meaning_index().meaning_for(normalized_tail))
    ...
    def _agree_adjective_with_previous_noun(
        self,
        adjective: str,
        previous_word: str | None,
    ) -> str:
        """Apply only dictionary-proven adjacent noun/adjective agreement."""
        adjective_norm = self._normalize_word(adjective)
        previous_norm = self._normalize_word(previous_word or "")
        if not adjective_norm or not previous_norm:
            return adjective

        noun_tags = self.word_tags.get(previous_norm, set())
        adjective_tags = self.word_tags.get(adjective_norm, set())
        if not any(tag.startswith("SINGNOUNF-") for tag in noun_tags):
            return adjective

        masculine_glosses = {
            tag.split("-", 1)[1]
            for tag in adjective_tags
            if tag.startswith("SINGADJM-") and "-" in tag
        }
        if not masculine_glosses:
            return adjective

        feminine_candidate = adjective_norm + "a"
        feminine_glosses = {
            tag.split("-", 1)[1]
            for tag in self.word_tags.get(feminine_candidate, set())
            if tag.startswith("SINGADJF-") and "-" in tag
        }
        if not masculine_glosses.intersection(feminine_glosses):
            return adjective

        return self._match_capitalisation(adjective, feminine_candidate)
    ...
    def _maltese_cultural_name_set(self) -> frozenset[str]:
        if self._maltese_cultural_names is None:
            self._maltese_cultural_names = self._load_tagged_auxiliary_word_list(
                self._auxiliary_word_list_paths(NAMES_DIC.name),
                {"MLTNAME"},
            )
        return self._maltese_cultural_names
    ...
    def _capitalized_name_kind(self, word: str) -> str | None:
        ...
        if normalized in self._given_name_set():
            return "NAME"
        if normalized in self._maltese_cultural_name_set():
            return "MLTNAME"
    ...
    def _capitalized_name_repair(self, word: str) -> str | None:
        ...
        if not normalized:
            return None
        if normalized in self.dictionary_set or self.word_tags.get(normalized):
            return None

        names = (
            self._given_name_set()
            | self._maltese_cultural_name_set()
            | self._surname_set()
        )
        candidates: list[str] = []
        ...
        for index in range(len(graphemes) - 1):
            swapped = list(graphemes)
            swapped[index], swapped[index + 1] = swapped[index + 1], swapped[index]
            add(self._from_graphemes(swapped))
        for index in range(len(graphemes) + 1):
            add(self._from_graphemes(graphemes[:index] + ["h"] + graphemes[index:]))
        for index, grapheme in enumerate(graphemes):
            if grapheme == "h":
                add(self._from_graphemes(graphemes[:index] + graphemes[index + 1 :]))
    ...
    def _dictionary_internal_vowel_variants(self, word: str) -> list[str]:
        """Return dictionary-backed internal i/e and o/u repairs."""
        normalized = self._normalize_word(word)
        graphemes = list(self._graphemes(normalized))
        if len(graphemes) < 4:
            return []
        ...
        for index in range(1, len(graphemes) - 1):
            replacements = list(swaps.get(graphemes[index], ()))
            if (
                graphemes[index] == "e"
                and index > 0
                and graphemes[index - 1] == "għ"
            ):
                replacements.append("a")
            for replacement in replacements:
                candidate = self._from_graphemes(
                    graphemes[:index] + [replacement] + graphemes[index + 1 :]
                )
                if candidate in self.dictionary_set or self._valid_suffix_surface_candidates(candidate):
                    if candidate not in matches:
                        matches.append(candidate)
                    continue
                tags = self.word_tags.get(candidate, set())
                if any(
                    str(tag).startswith(
                        (
                            "SINGNOUN",
    ...
    def _exact_lowercase_proper_name(self, word: str) -> str | None:
        ...
        if orthographic is not None:
            for variant in orthographic.shortcut_letter_variants(
                ...
                if variant in self.country_maltese_to_english:
                    return self.place_word_display.get(
                        variant,
                        self._capitalize_first_letter(variant),
                    )
        if normalized in self._calendar_proper_names:
    ...
    def _noun_possessive_stems(self, noun: str) -> list[tuple[str, bool]]:
        ...
        if normalized.endswith("'"):
            ...
            if (
                len(graphemes) >= 4
                and graphemes[-2] == "e"
                and graphemes[-1] not in self.VOWELS
            ):
                add(
                    self._from_graphemes(
                        graphemes[:-2] + graphemes[-1:]
                    ),
                    False,
                )
    ...
    def _looks_like_noun_possessive_surface(self, word: str) -> bool:
        normalized = self._normalize_word(word)
        if not normalized or "-" in normalized or "'" in normalized:
            return False
        suffixes = ("kom", "hom", "ek", "ok", "ha", "na", "i", "u", "h", "k")
        suffix = next((s for s in suffixes if normalized.endswith(s)), "")
        if not suffix or len(normalized) <= len(suffix) + 1:
            return False
        stem = normalized[: -len(suffix)]
        if not any(ch in self.VOWELS for ch in stem):
            return False
        if len(suffix) == 1 and len(stem) < 3:
            return False
        return True

    def _has_noun_possessive_repair_cue(self, word: str) -> bool:
        normalized = self._normalize_word(word)
        if not self._looks_like_noun_possessive_surface(normalized):
            return False
        if self._has_strong_auto_repair_cue(normalized):
            return True
        if re.search(r"(jj|ww)[^aeiouàèìòù]", normalized):
            return True
        if normalized.endswith(("tna", "tkom", "thom")):
            return True
        return False

    def _correct_noun_possessive_suffix(self, word: str) -> str | None:
        normalized = self._normalize_word(word)
        cache = self._request_noun_possessive_correct_cache()
        if normalized in cache:
            return cache[normalized]
        ...
        if not self._has_noun_possessive_repair_cue(normalized):
        ...
        if normalized.endswith("a") and any(
            marker in normalized for marker in ("h", "ħ", "għ", "gh")
        ):
    ...
    def _collapse_invalid_glide_doubling(self, word: str) -> str | None:
        ...
        orthographic_generator = getattr(self, "orthographic_generator", None)
        for candidate in candidates:
            for suffix_repair in self._suffix_repair_variants(candidate):
                suffix_variants = [suffix_repair]
                if suffix_repair.startswith("i") and len(suffix_repair) > 1:
                    suffix_variants.append("j" + suffix_repair[1:])
                if any(
                    self._valid_suffix_surface_candidates(variant)
                    for variant in suffix_variants
                ):
                    return suffix_repair
            if (
                candidate in self.dictionary_set
    ...
    def _load_places_dictionary(self, file_path: Path) -> None:
        ...
        for entry in entries:
            ...
            if " " in normalized:
                ...
                self.place_phrase_first_words.add(normalized.split()[0])
                self.place_phrase_shortcut_display.setdefault(
                    self._strip_maltese_shortcuts(normalized),
                    entry,
                )
    ...
    def _add_contextual_cluster_confusion_choices(
        self,
        tokens: list[dict],
    ) -> None:
        """Expose bounded stop-consonant alternatives to the context selector.

        The ordinary word fixer intentionally prefers the cheapest standalone
        repair (for example ``gbir -> ġbir``).  In a sentence, however, a
        clustered voiced/unvoiced typo can produce another real word, as in
        ``ħabib gbir -> ħabib kbir``.  Add that alternative only when
        sentence context or a noun-adjective POS pattern supports it; the
        corpus selector still makes the final choice.
        """
        lexical_indexes = [
            index
            for index, token in enumerate(tokens)
            if isinstance(token, dict) and token.get("type") in {"word", "phrase"}
        ]
        if len(lexical_indexes) < 2:
            return
        ...
        article_rules = getattr(self, "article_phrase_rules", None)
        selector = getattr(self, "corpus_context_selector", None)
        scorer = getattr(selector, "scorer", None)

        for position, token_index in enumerate(lexical_indexes):
            token = tokens[token_index]
            if token.get("type") != "word" or token.get("name_like"):
                continue

            original = self._normalize_word(str(token.get("original", "")))
            current = self._normalize_word(str(token.get("corrected", "")))
            graphemes = self._graphemes(original)
            if len(graphemes) < 2:
                continue

            previous = (
                str(tokens[lexical_indexes[position - 1]].get("corrected", ""))
                if position > 0
                else ""
            )
            next_surface = (
            ...
            for index, letter in enumerate(graphemes):
                replacement = pair_map.get(letter)
                ...
                candidate = self._normalize_word("".join(changed))
                if (
                    candidate != current
            ...
            for candidate in alternatives:
                ...
                if scorer is not None and scorer.is_available():
                    candidate_score = scorer.score_candidate_components(
                        candidate,
                        prev_word=previous or None,
                        next_word=next_surface or None,
                    )
                    current_score = scorer.score_candidate_components(
                        current,
                        prev_word=previous or None,
                        next_word=next_surface or None,
                    )
                    has_context = any(
                        float(candidate_score.get(key, 0.0)) > 0.0
                        for key in ("left_bigram", "right_bigram", "trigram")
                    )
                    context_supported = has_context and (
                        float(candidate_score.get("total_before_cap", 0.0))
                        > float(current_score.get("total_before_cap", 0.0))
                    )

                if not context_supported and article_rules is not None and previous:
                    context_supported = bool(
                        article_rules.is_noun(previous)
                        and self._is_adjective_tagged_word(candidate)
                        and not self._is_adjective_tagged_word(current)
                    )
                if not context_supported:
                    continue

                choices = token.setdefault("choices", [])
                if any(
                    self._normalize_word(str(choice.get("word", ""))) == candidate
                ...
                choices.append(
                    {
                        "word": candidate,
                        "meaning": "",
                        "suggestion_kind": "contextual_cluster_confusion",
                    }
                )
    ...
    def _candidate_score(self, typo: str, candidate: str, stage: str) -> ScoreRow:
        ...
        candidate_norm = self._normalize_word(candidate)
        if (
            len(self._letter_tokens(typo_norm)) <= 4
            and not typo_norm.endswith("x")
            and candidate_norm.endswith("x")
            and len(self._letter_tokens(candidate_norm)) - len(self._letter_tokens(typo_norm)) >= 2
        ):
            return ScoreRow(candidate_norm, 9.0, 99, 1.0, 1.0, 1.0, 1.0, stage, typo_norm)
        cache = self._request_candidate_score_cache()
        cache_key = (typo_norm, candidate_norm, stage)
        cached = cache.get(cache_key)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["candidate_score"] = (
                    profiler.cache_hits.get("candidate_score", 0) + 1
                )
            return cached
        ...
        if profiler:
            profiler.cache_misses["candidate_score"] = (
                profiler.cache_misses.get("candidate_score", 0) + 1
            )
        return best
    ...
    def _guttural_vowel_metathesis_suffix_repairs(self, word: str) -> list[str]:
        """Move a typed vowel from after gh/għ to before it, then reparse."""
        normalized = self._normalize_word(word)
        match = re.match(r"^([jtn])(?:gh|għ)([aeiou])(.*)$", normalized)
        if not match:
            return []

        prefix, vowel, tail = match.groups()
        candidates: list[str] = []
        for guttural in ("gh", "għ"):
            seed = f"{prefix}{vowel}{guttural}{tail}"
            for candidate in (seed, self._correct_suffix_cached(seed)):
                candidate = self._normalize_word(candidate or "")
    ...
    def _pronominal_suffix_glide_variants(self, word: str) -> list[str]:
        """Remove an intrusive j before the productive -hom object suffix."""
        normalized = self._normalize_word(word)
        variants: list[str] = []
        for typed in ("ijhom", "ijom"):
            if not normalized.endswith(typed):
                continue
            candidate = normalized[: -len(typed)] + "ihom"
            if (
                self._ta_direct_object_gloss(candidate)
                or self._is_verb_tagged_word(candidate)
                or self._valid_suffix_surface_candidates(candidate)
            ):
                variants.append(candidate)
    ...
    def _missing_pronominal_h_variants(self, word: str) -> list[str]:
        """Restore an omitted h only at a validated object-suffix boundary."""
        normalized = self._normalize_word(word)
        candidates: list[str] = []

        if normalized.endswith("a") and len(normalized) > 2:
            candidates.append(normalized[:-1] + "ha")
        if normalized.endswith("om") and not normalized.endswith("hom"):
            candidates.append(normalized[:-2] + "hom")

        return [
            candidate
            for candidate in dict.fromkeys(candidates)
            if self._exact_suffix_matches_cached(candidate)
            and self._suffix_anchor_compatible(normalized, candidate)
        ]

    def _preferred_ido_l_surface(self, word: str) -> str | None:
        """Restore the stem-final l before -lek/-lok when the IDO parse exists."""
        normalized = self._normalize_word(word)
        negative = "x" if normalized.endswith("x") else ""
        body = normalized[:-1] if negative else normalized
        for ending in ("lek", "lok"):
            if not body.endswith(ending):
                continue
            stem = body[: -len(ending)]
            if not stem or stem.endswith("l"):
                continue
            candidate = f"{stem}l{ending}{negative}"
            if any(
                getattr(parsed, "suffix_kind", "") == "IDO"
                and getattr(parsed, "suffix_person", "") == "2S"
                for parsed in self._valid_suffix_surface_candidates(candidate)
            ):
                return candidate
        return None
    ...
    def _validated_liex_to_lix_repair(self, word: str) -> str | None:
        ...
        if suffix_generator is not None and self._exact_suffix_matches_cached(candidate):
            return candidate
    ...
    def _valid_generated_surface(self, candidate: str) -> bool:
        ...
        if suffix_generator is None:
            return False
        return bool(
            self._exact_suffix_matches_cached(normalized)
            or self._normalize_word(self._correct_suffix_cached(normalized) or "")
            == normalized
        )
    ...
    def _supports_verb_final_gh_repair(self, candidate: str) -> bool:
        ...
        matches = self._exact_suffix_matches_cached(normalized)
        if matches:
            return True
        generated = self._correct_suffix_cached(normalized)
    ...
    def _final_object_suffix_variants(self, word: str) -> list[str]:
        ...
        if (
            ...
            if (
                ...
                or (
                    suffix_generator is not None
                    and bool(self._exact_suffix_matches_cached(plain))
                )
            ):
    ...
    def _restore_direct_object_guttural_suffix(self, word: str) -> list[str]:
        """Restore common guttural material before -ha/-hom object suffixes."""
        normalized = self._normalize_word(word)
        if not normalized:
            return []

        variants: list[str] = []
        if normalized.endswith("ahha") and len(normalized) > 4:
            variants.append(normalized[:-4] + "agħha")
        if normalized.endswith("ahhom") and len(normalized) > 5:
            variants.append(normalized[:-5] + "agħhom")
        ...
        if orthographic_generator is not None and hasattr(
            ...
            for seed in (normalized, *tuple(variants)):
                variants.extend(
                    orthographic_generator.shortcut_letter_variants(
                        seed,
                        max_changes=2,
    ...
    def _hollow_ju_suffix_vowel_repair(self, word: str) -> str | None:
        """Repair jo+r... surfaces to exact generated ju+r... verb forms."""
        normalized = self._normalize_word(word)
        if not normalized.startswith("jor") or len(normalized) < 5:
            return None
        candidate = "ju" + normalized[2:]
        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is None:
            return None
        if self._exact_suffix_matches_cached(candidate):
            return candidate
        if candidate.endswith("khom"):
            without_intrusive_h = candidate[:-4] + "kom"
            if self._exact_suffix_matches_cached(without_intrusive_h):
                return without_intrusive_h
        return None
    ...
    def _get_candidates_cached(self, normalized: str) -> tuple[str, ...]:
        ...
        # 5. Final consonant voicing swap (e.g., bajt <-> bajd)
        voicing_swaps = {
            "t": "d", "d": "t", "p": "b", "b": "p",
            "k": "g", "g": "k", "f": "v", "v": "f",
            "s": "z", "z": "s", "ċ": "ġ", "ġ": "ċ"
        }
        if len(normalized) >= 3 and normalized[-1] in voicing_swaps:
            swapped = normalized[:-1] + voicing_swaps[normalized[-1]]
            if swapped in self.dictionary_set:
                add(swapped)

        # 6. Final silent apostrophe for weak/għ verbs (e.g., tefa -> tefa', qara -> qara')
        if normalized and normalized[-1] in "aeiou":
            apostrophe_cand = normalized + "'"
            if apostrophe_cand in self.dictionary_set:
                add(apostrophe_cand)
    ...
    def _ensure_symspell_index(
        self,
        *,
        allow_blocking_build: bool = SYMSPELL_BLOCKING_BUILD,
    ) -> MalteseSymSpellIndex | None:
        index = getattr(self, "symspell_index", None)
        if index is None:
            return None
        if index.stats.built:
            return index
        if not allow_blocking_build:
            profiler = current_profiler()
            if profiler is not None:
                profiler.increment("symspell_build_skipped")
            return None
        with self._symspell_lock:
            if not index.stats.built:
                symspell_stats = index.build(self.dictionary)
                log_spellcheck_event(
    ...
    def _symspell_candidates(self, word: str, *, limit: int = SYMSPELL_MAX_RESULTS) -> list[str]:
        index = self._ensure_symspell_index()
        if index is None:
            return []
        normalized = self._normalize_word(word)
        profiler = current_profiler()
    ...
    def _build_candidate_evidence_pool(
        self,
        word: str,
        *,
        lexicalized_forms: Iterable[str] = (),
    ) -> CandidateEvidencePool:
        normalized = self._normalize_word(word)
        lexicalized_key = tuple(
            self._normalize_word(variant)
            for variant in lexicalized_forms
            if self._normalize_word(variant)
        )
        cache_key = (normalized, lexicalized_key)
        cache = self._request_candidate_evidence_pool_cache()
        cached = cache.get(cache_key)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["candidate_evidence_pool"] = (
                    profiler.cache_hits.get("candidate_evidence_pool", 0) + 1
                )
            return cached

        pool = CandidateEvidencePool(normalizer=self._normalize_word)
        ...
        if ENABLE_SYMSPELL_CANDIDATES and symspell_candidates and not SYMSPELL_SHADOW_MODE:
            pool.add_many(
                ...
                confidence="medium",
                reason="delete-index nearby word",
            )
        else:
            legacy_candidates = self._get_candidates(normalized)
            pool.add_many(
                legacy_candidates,
                "legacy-anchor",
                candidate_type="lexical",
                confidence="medium",
                reason="consonant/vowel anchor search",
            )
            pool.add_many(
                symspell_candidates,
                "symspell-shadow",
                candidate_type="lexical-shadow",
                confidence="shadow",
                reason="delete-index nearby word",
            )

        for variant in lexicalized_forms:
            variant_norm = self._normalize_word(variant)
            variant_symspell = self._symspell_candidates(variant_norm)
            if ENABLE_SYMSPELL_CANDIDATES and variant_symspell and not SYMSPELL_SHADOW_MODE:
                pool.add_many(
                    variant_symspell,
                    "symspell-lexicalized",
                    candidate_type="lexical",
                    confidence="medium",
                    reason=f"delete-index after {variant_norm}",
                )
            else:
                variant_legacy = self._get_candidates(variant_norm)
                pool.add_many(
                    variant_legacy,
                    "legacy-lexicalized",
                    candidate_type="lexical",
                    confidence="medium",
                    reason=f"anchor search after {variant_norm}",
                )
                pool.add_many(
                    variant_symspell,
                    "symspell-shadow-lexicalized",
                    candidate_type="lexical-shadow",
                    confidence="shadow",
                    reason=f"delete-index after {variant_norm}",
                )
        ...
        if ortho_gen is not None:
            for candidate in ortho_gen.shortcut_letter_variants(
                normalized,
                max_changes=1,
                max_variants=24,
            ):
                if self._normalize_word(candidate) in self.dictionary_set:
                    pool.add(
                        candidate,
                        "shortcut-letter",
                        candidate_type="orthographic",
                        confidence="high",
                        reason="Maltese keyboard shortcut",
                    )
            combined_variants = []
            ...
            for combined in combined_variants:
                if self._normalize_word(combined) in self.dictionary_set:
                    pool.add(
                        ...
                        candidate_type="orthographic",
                        confidence="high",
                        reason="voicing plus silent-letter restoration",
                    )
        for candidate in self._dictionary_verified_basic_repairs(normalized):
            pool.add(
                candidate,
                "phase-y-basic-repair",
                candidate_type="structural",
                confidence="high",
                reason="bounded combined basic repairs",
            )
        for candidate in self._medial_guttural_vowel_restoration_variants(normalized):
            pool.add(
                candidate,
                "medial-guttural-vowel",
                candidate_type="structural",
                confidence="high",
                reason="restore VhV/VgħV vowel structure",
            )
        for candidate in self._short_missing_guttural_vowel_variants(normalized):
            pool.add(
                candidate,
                "short-guttural-vowel",
                candidate_type="structural",
                confidence="high",
                reason="restore short C...VC...C guttural-vowel loss",
            )
        for candidate in self._restore_direct_object_h_vowel(normalized):
            pool.add(
                candidate,
                "direct-object-h-vowel",
                candidate_type="suffix",
                confidence="high",
                reason="restore direct-object h vowel",
            )
        for candidate in self._missing_pronominal_h_variants(normalized):
            pool.add(
                candidate,
                "missing-pronominal-h",
                candidate_type="suffix",
                confidence="high",
                reason="restore omitted h at a validated object-suffix boundary",
            )
        for candidate in self._restore_direct_object_guttural_suffix(normalized):
            pool.add(
                candidate,
                "direct-object-guttural",
                candidate_type="suffix",
                confidence="high",
                reason="restore guttural before -ha/-hom",
            )
        hollow_ju = self._hollow_ju_suffix_vowel_repair(normalized)
        if hollow_ju:
            pool.add(
                hollow_ju,
                "hollow-ju-suffix",
                candidate_type="suffix",
                confidence="high",
                reason="restore jo/ju suffix vowel",
            )
        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is not None:
            corrected_suffix = self._correct_suffix_cached(normalized)
            if corrected_suffix and self._suffix_anchor_compatible(
                normalized,
                corrected_suffix,
            ):
                pool.add(
                    ...
                    candidate_type="suffix",
                    confidence="high",
                    reason="generated suffix correction",
                )
            for candidate in self._suggest_suffixes_cached(normalized, limit=8):
                if self._suffix_anchor_compatible(normalized, candidate):
                    pool.add(
                        candidate,
                        "suffix-generator-suggestion",
                        candidate_type="suffix",
                        confidence="medium",
        ...
        if profiler:
            profiler.cache_misses["candidate_evidence_pool"] = (
                profiler.cache_misses.get("candidate_evidence_pool", 0) + 1
            )
        return pool

    def _bounded_evidence_words_for_request(
        self,
        evidence_by_word: dict[str, CandidateEvidence],
        *,
        bulk_mode: bool,
    ) -> list[str]:
        if not bulk_mode or len(evidence_by_word) <= 48:
            return list(evidence_by_word)

        def priority(item: tuple[str, CandidateEvidence]) -> tuple[int, float, str]:
            word, evidence = item
            if evidence.candidate_types.intersection({"structural", "suffix"}):
                tier = 0
            elif "orthographic" in evidence.candidate_types:
                tier = 1
            elif any(source.startswith("symspell") for source in evidence.sources):
                tier = 2
            else:
                tier = 3
            return (tier, -evidence.confidence, word)

        ordered = sorted(evidence_by_word.items(), key=priority)
        return [word for word, _ in ordered[:48]]

    def _candidate_passes_hard_guards(
        self,
        typo: str,
        ...
        *,
        ...
        candidate = self._normalize_word(evidence.word)
        if not candidate:
            return False
        if candidate == normalized:
            return True
        if normalized in self.dictionary_set and not self._has_strong_auto_repair_cue(
            ...
            return False
        if self._accepted_exact_english(normalized):
            return False
        if self._is_implausible_vowel_swap(normalized, candidate):
            return False
        if (
            evidence.candidate_types.intersection({"structural", "suffix"})
            and row.edit_distance <= max_distance + 3
            and row.score <= 0.72
        ):
            return True
        if not self._is_acceptable_match(row, max_distance, score_limit=score_limit):
            return False
        if "symspell" in evidence.sources and evidence.sources == {"symspell"}:
            return row.edit_distance <= max_distance and row.score <= 0.50
        return True
    ...
    def _evidence_rank_key(
        ...
        source_bonus = 0.0
        if "orthographic" in evidence.candidate_types:
            source_bonus += 0.08
        if "apertium" in evidence.sources:
            source_bonus += 0.03
        if any(source.startswith("symspell-shadow") for source in evidence.sources):
            source_bonus -= 0.08
        adjusted_score = row.score - min(0.12, evidence.confidence * 0.08 + source_bonus)
        return (adjusted_score, row.edit_distance, row.candidate)

    def candidate_evidence_debug(self, word: str, *, limit: int = 12) -> dict:
        normalized = self._normalize_word(word)
        pool = self._build_candidate_evidence_pool(normalized)
        max_distance = self._max_distance(normalized)
        evidence_by_word = pool.by_word()
        ranked = []
        for candidate, evidence in evidence_by_word.items():
            row = self._candidate_score(normalized, candidate, stage="debug-evidence")
            accepted = self._candidate_passes_hard_guards(
                normalized,
                evidence,
                row,
            ...
            ranked.append(
                {
                    ...
                    "rank_key": self._evidence_rank_key(row, evidence),
                }
            )
        ranked.sort(key=lambda item: item["rank_key"])
        for item in ranked:
            item["rank_key"] = [
                round(item["rank_key"][0], 4),
                item["rank_key"][1],
                item["rank_key"][2],
            ]
        return {
            ...
            "symspell_enabled": ENABLE_SYMSPELL_CANDIDATES,
            "symspell_shadow": SYMSPELL_SHADOW_MODE,
            "apertium_enabled": ENABLE_APERTIUM_ANALYZER,
            "candidates": pool.diagnostics(limit=limit),
            "ranked": ranked[:limit],
        }

    def correction_trace(self, word: str, *, limit: int = 12) -> dict:
        """Return the full bounded-candidate path used for one surface."""
        ...
        return {
            ...
            "phase_corrected": analysis.corrected,
            "basic_candidates": list(analysis.basic_candidates),
            "complex_candidates": list(analysis.complex_candidates),
            "final_correction": corrected,
            "suggestions": self.suggest(word, limit=limit),
            "recognition_sources": list(
                self._surface_recognition_sources(corrected)
            ),
            "evidence": self.candidate_evidence_debug(word, limit=limit),
        }
    ...
    def _fixed_time_phrase_match(
        ...
        if current_norm == "bil" and next_norm in {
            "qeda",
            "qedha",
            "qedgha",
            "qedgħa",
            ...
            corrected = display("bilqiegħda")
            return corrected, [
                {"word": corrected, "meaning": self.meaning_for("bilqiegħda")}
            ], 2
        ...
        *,
        ...
        if original_norm.endswith("ha") or original_norm.endswith("ħa"):
            ...
            if candidate_norm == stem + "aha":
                return True
        if original_norm.endswith("om") and not original_norm.endswith("hom"):
            if candidate_norm == original_norm[:-2] + "hom":
                return True
    ...
    def _normalize_currency_amounts(self, text: str) -> str:
        def replace(match: re.Match[str]) -> str:
            amount = match.group("prefix") or match.group("suffix") or "0"
            amount = amount.replace(",", ".")
            whole, separator, decimal = amount.partition(".")
            if not separator:
                decimal = "00"
            else:
        ...
        return self.EURO_AMOUNT_PATTERN.sub(replace, text)
    ...
    def _ensure_terminal_period(self, text: str) -> str:
        ...
        # A blank line closes a prose paragraph. Apply the same terminal
        # punctuation rule to every completed paragraph, not only to the
        # final one in the request.
        paragraph_parts = re.split(r"(\r?\n[ \t]*\r?\n)", text)
        for index in range(0, len(paragraph_parts) - 1, 2):
            paragraph = paragraph_parts[index]
            stripped_paragraph = paragraph.rstrip()
            if (
                stripped_paragraph
                and stripped_paragraph[-1] not in ".?!"
                and not (
                    len(stripped_paragraph) >= 2
                    and stripped_paragraph[-1] in "'\"\u2019\u201d"
                    and stripped_paragraph[-2] in ".?!"
                )
            ):
                trailing = paragraph[len(stripped_paragraph) :]
                paragraph_parts[index] = f"{stripped_paragraph}.{trailing}"
    ...
    def _correct_place_word(self, word: str) -> str | None:
        ...
        if not self._is_initial_capitalized(str(word)):
            return None
    ...
    def _epenthetic_place_base(self, word: str) -> str | None:
        """Return the canonical bare place behind an L-I... place form."""
        normalized = self._normalize_word(word)
        if not normalized:
            return None
        if (
            normalized in self.place_word_set
            and f"l-i{normalized}" in self.place_word_set
        ):
            return normalized
        if (
            normalized.startswith("i")
            and normalized[1:] in self.place_word_set
            and f"l-{normalized}" in self.place_word_set
        ):
            return normalized[1:]
        return None

    def _epenthetic_place_surface(
        self,
        word: str,
        *,
        prefer_initial_i: bool,
    ) -> str | None:
        base = self._epenthetic_place_base(word)
        if base is None:
            return None
        display = self.place_word_display.get(base, self._capitalize_first_letter(base))
        if not prefer_initial_i:
            return display
        return f"I{display[:1].lower()}{display[1:]}"

    def _is_recognized_surface(self, word: str) -> bool:
        normalized = self._normalize_word(word)
        if not normalized:
            return True
        cache = self._request_recognized_surface_cache()
        cache_key = word
        cached = cache.get(cache_key)
        profiler = current_profiler()
        if cached is not None:
            if profiler:
                profiler.cache_hits["recognized_surface"] = (
                    profiler.cache_hits.get("recognized_surface", 0) + 1
                )
            return cached

        result = self._is_recognized_surface_uncached(word, normalized)
        cache[cache_key] = result
        if profiler:
            profiler.cache_misses["recognized_surface"] = (
                profiler.cache_misses.get("recognized_surface", 0) + 1
            )
        return result

    def _is_recognized_surface_uncached(self, word: str, normalized: str) -> bool:
        if any(
            normalized in alternatives
            for alternatives in self.FIXED_TIME_ALTERNATIVES.values()
        ):
            return True
        ...
        if normalized in self.dictionary_set:
            return True
        if self._preposition_pronoun_suffix_base(normalized):
            return True
        ...
        if self._is_recognized_hyphenated_compound(normalized):
            return True
        if self._is_recognized_apostrophe_compound(normalized):
            return True
        if self._valid_suffix_surface_candidates(normalized):
            return True
        return any(
    ...
    def _is_recognized_apostrophe_compound(self, word: str) -> bool:
        normalized = self._normalize_word(word)
        if "'" not in normalized:
            return False
        prefix, tail = normalized.split("'", 1)
        if prefix not in {"b", "f", "m", "x", "l", "s", "t"} or not tail:
            return False
        return any(
            (
                self._accepted_exact_english(tail),
                tail in self.dictionary_set,
                bool(self.word_tags.get(tail)),
                self._is_recognized_surface(tail),
            )
        )
    ...
    def _is_recognized_hyphenated_compound(self, word: str) -> bool:
        ...
        if not prefix or not tail:
            return False
        tail_is_valid = self._valid_generated_surface(tail) or self._accepted_exact_english(tail)
        ...
        if prefix in {
            "bl", "fl",
            "bil", "biċ", "bic", "bid", "bin", "bir", "bis", "bit", "bix", "biż", "biz",
            "fil", "fiċ", "fic", "fid", "fin", "fir", "fis", "fit", "fix", "fiż", "fiz",
            "mal", "maċ", "mac", "mad", "man", "mar", "mas", "mat", "max", "maż", "maz",
            "mill", "miċ", "mic", "mid", "min", "mir", "mis", "mit", "mix", "miż", "miz",
            "tal", "taċ", "tac", "tad", "tan", "tar", "tas", "tat", "tax", "taż", "taz",
            "għall", "għaċ", "għac", "għad", "għan", "għar", "għas", "għat", "għax", "għaż", "għaz",
            "bħall", "bħaċ", "bhac", "bħad", "bhad", "bħan", "bhan", "bħar", "bhar",
            "bħas", "bhas", "bħat", "bhat", "bħax", "bhax", "bħaż", "bhaz",
        }:
            return any(
                (
                    self._accepted_exact_english(tail),
        ...
        if prefix in {"il", "l", "iċ", "ic", "id", "in", "ir", "is", "it", "ix", "iż", "iz"}:
            return any(
                (
                    self._accepted_exact_english(tail),
                    self._is_noun_tagged_word(tail),
    ...
    def _surface_recognition_sources(self, surface: str) -> tuple[str, ...]:
        normalized = self._normalize_word(surface)
        if not normalized:
            return ()
        ...
        if re.fullmatch(r"\d+(?:[.,]\d+)*", normalized):
            add("number")
        if self._accepted_exact_english(surface):
            add("exact-english")
        if normalized in self.dictionary_set:
            add("dictionary")
        if self.word_tags.get(normalized):
            add("tagged")
        if self._preposition_pronoun_suffix_base(normalized):
            add("preposition-suffix")
        if self._is_initial_capitalized(surface):
            if (
                normalized in self._protected_name_set()
                or normalized in self._given_name_set()
                or normalized in self._surname_set()
            ):
                add("name")
        if normalized in self.place_word_set or self._exact_place_word(surface):
            add("place")
        if self._correct_noun_possessive_suffix(normalized) == normalized:
            add("noun-possessive")
        if self._valid_suffix_surface_candidates(normalized):
        ...
        if (
            self._is_recognized_hyphenated_compound(normalized)
            or self._is_recognized_apostrophe_compound(normalized)
        ):
            add("article-preposition")
        if self._is_recognized_surface(surface):
            add("recognized-surface")
    ...
    def _mark_unrecognized_tokens(self, tokens: list[dict]) -> None:
        for token in tokens:
            if token.get("type") != "word":
                continue
            forced = bool(token.pop("force_unrecognized", False))
            corrected = str(token.get("corrected", "")).strip()
            original = str(token.get("original", "")).strip()
            sources = list(self._surface_recognition_sources(corrected))
            if sources:
                token["recognition_sources"] = sources

            if forced and not sources:
            ...
            if original and self._normalize_word(original) != self._normalize_word(corrected):
                token["unrecognized"] = False
                if "corrected-output" not in sources:
                    sources.append("corrected-output")
                    token["recognition_sources"] = sources
                continue
            token["unrecognized"] = bool(corrected) and not bool(sources)
    ...
    def _match_capitalized_place_phrase(
        ...
        def crosses_sentence_boundary(consumed: int) -> bool:
            return any(
                any(mark in text[matches[pos].end() : matches[pos + 1].start()] for mark in ".?!")
        ...
        # Exact place phrases retain their dictionary capitalization even
        # when every input token is lowercase. Match longest first so a short
        # place such as San Pawl does not consume the start of a longer name.
        max_words = min(8, len(word_tokens) - index)
        if current_normalized in self.place_phrase_first_words:
            for consumed in range(max_words, 1, -1):
                if crosses_sentence_boundary(consumed):
                    continue
                original_phrase = text[
                    matches[index].start() : matches[index + consumed - 1].end()
                ]
                exact_key = self._normalize_word(original_phrase)
                exact_display = self.place_phrase_display.get(exact_key)
                if exact_display:
                    return original_phrase, exact_display, consumed
                strict_display = self.place_phrase_shortcut_display.get(
                    self._strip_maltese_shortcuts(exact_key)
                )
                if strict_display:
                    return original_phrase, strict_display, consumed
        ...
        if index + 1 < len(word_tokens):
            ...
            if crosses_sentence_boundary(2):
                return None
            original_phrase = text[
        ...
        if index + 2 < len(word_tokens) and current_capitalized:
            if crosses_sentence_boundary(3):
                return None
    ...
    def _blocks_personal_name_gh_repair(
        self,
        original_word: str,
        candidate: str,
        *,
        ...
        """Do not invent għ inside an unknown mid-sentence personal name."""
        original_norm = self._normalize_word(original_word)
        if (
            sentence_initial
            or not self._is_initial_capitalized(original_word)
            or original_word.isupper()
            or original_norm in self.EXACT_SUGGESTION_OVERRIDES
        ):
            return False
        candidate_norm = self._normalize_word(candidate)
        if "għ" not in candidate_norm or "għ" in original_norm or "gh" in original_norm:
            return False
        return not bool(
            self._exact_place_word(candidate)
            or candidate_norm in self.place_word_set
            or candidate_norm in self.country_maltese_to_english
        )
    ...
    def _is_future_particle_complement(self, word: str) -> bool:
        ...
        if not records and suffix_generator is not None:
            records = [
                record
                for candidate in self._exact_suffix_matches_cached(normalized)
                for record in verb_index.word_records(candidate.base)
            ]
    ...
    def _initial_vowel_surface_options(
        ...
        if normalized.startswith("i") and (
            ...
            or (
                suffix_generator is not None
                and bool(self._exact_suffix_matches_cached(normalized))
            )
        ) and not self._is_initial_vowel_base_candidate(normalized[1:]):
        ...
        if normalized.startswith("i") and len(normalized) > 1:
            ...
            if (
                ...
                and (
                    ...
                    or self._initial_i_participial_adjective_surface(plain_cluster)
                )
            ):
        ...
        if (
            ...
            and (
                ...
                or self._initial_i_participial_adjective_surface(normalized)
            )
        ):
    ...
    def _split_article_unknown_tail(
        ...
        *,
        ...
        def with_tail_case(surface: str) -> str:
            return self._match_hyphenated_tail_capitalisation(tail, surface)
        ...
        definite_articles = {
            "il", "l", "ic", "iċ", "id", "in", "ir", "is", "it", "ix", "iz", "iż", "i",
        }
        article_prepositions = {
            "tal", "mal", "bil", "fil", "fis", "lil", "lill", "lir", "lit", "lid",
            "lin", "liċ", "lic", "lis", "liż", "liz", "lix", "mill", "għall",
            "ghall", "bħall", "bhall", "sal", "mic", "miċ",
        }

        if article_norm in definite_articles:
            return with_tail_case(
                article_rules.corrected_article_phrase(
                    article_norm,
                    tail_norm,
                    previous,
                )
        ...
        if article_norm in article_prepositions:
            ...
            if assimilated is not None:
                ...
                return with_tail_case(f"{surface_prefix}-{tail_norm}")
            preposition_stems = {
                "tal": "ta",
                "mal": "ma",
                "bil": "bi",
            ...
            if first in sun_letters and article_norm in preposition_stems:
                return with_tail_case(
                    f"{preposition_stems[article_norm]}{first}-{tail_norm}"
                )
            return with_tail_case(
                f"{canonical_prefixes.get(article_norm, article_norm)}-{tail_norm}"
            )
        return None
    ...
    def _correct_inline_article_word(
        ...
        *,
        ...
        if (
            prefix in {"il", "l"}
            and corrected_noun_norm
            and corrected_noun_norm != noun
            and self._is_recognized_surface(corrected_noun_norm)
        ):
            return f"{prefix}-{corrected_noun_norm}"
        return None
    ...
    def _noun_meaning_keys(self, word: str) -> set[str]:
        keys: set[str] = set()
        for meaning in self._get_meaning_index().meanings_for(word):
    ...
    def _number_phrase_payload(
        ...
        *,
        ...
        noun_surface_norm = self._normalize_word(noun_surface_word)
        noun_base = noun_surface_norm
        if noun_surface_norm.startswith("i") and len(noun_surface_norm) > 1:
            possible_base = noun_surface_norm[1:]
            if (
                self._word_starts_with_two_consonants(possible_base)
                and self._is_noun_tagged_word(possible_base)
            ):
                noun_base = possible_base
        corrected_noun = self._normalize_word(self.correct_word(noun_base))
        ...
        if has_short_attnum:
            if article_prefix is not None and has_ordnum:
                ...
                ordinal_meaning = self._get_meaning_index().meaning_for(ordinal_surface).split("/", 1)[0].strip()
                if not ordinal_meaning or ordinal_meaning == "the":
                    ordinal_meaning = self._get_meaning_index().meaning_for(numeral_norm).split("/", 1)[0].strip()
                corrected_meaning = self._get_meaning_index().meaning_for(corrected_noun)
                alternate_meaning = self._get_meaning_index().meaning_for(alternate_noun)
    ...
    def _expand_compact_xi_article(self, word: str) -> str | None:
        ...
        # A complete lexical shortcut such as xih -> xiħ is one word, not
        # ``xi`` followed by a one-letter article tail.
        if any(
            variant in self.dictionary_set
            for variant in self._strict_lookup_variants(normalized)
        ) or (
            hasattr(self, "orthographic_generator")
            and bool(
                self.orthographic_generator.dictionary_shortcut_variants(normalized)
            )
        ):
            return None
    ...
    def _expand_compact_long_preposition_phrase(self, word: str) -> str | None:
        ...
        for compact_prefix, long_prefix in prefix_map.items():
            ...
            if not tail:
                return None
            if compact_prefix == "x'":
                repaired_tail = self._article_tail_repair(tail) or tail
                corrected_tail = self._normalize_word(self.correct_word(repaired_tail))
                if (
                    self._is_recognized_surface(repaired_tail)
                    or self._is_recognized_surface(corrected_tail)
                    or bool(self._valid_suffix_surface_candidates(repaired_tail))
                    or bool(self._valid_suffix_surface_candidates(corrected_tail))
                ):
                    return None
            if compact_prefix == "l'":
    ...
    def _pronominal_suffix_gliding_repair(self, word: str) -> str | None:
        normalized = self._normalize_word(word)
        if "j" not in normalized or len(normalized) < 5:
            return None
        if normalized in self.dictionary_set or self._is_recognized_surface(normalized):
            return None

        patterns = [
            ("julix", "hulix"), ("julix", "ħulix"),
            ("julhom", "hulhom"), ("julhom", "ħulhom"),
            ("jalix", "halix"), ("jalix", "ħalix"),
            ("jalhom", "halhom"), ("jalhom", "ħalhom"),
            ("juli", "huli"), ("juli", "ħuli"),
            ("jijx", "hix"), ("jijx", "ħix"),
            ("jumx", "humx"), ("jumx", "ħumx"),
            ("jielu", "hielu"), ("jielu", "ħielu"),
        ]
        ...
        import re
        if not normalized.endswith("ija"):
            gen_h = re.sub(r'([aeiou])j([uai])', r'\1h\2', normalized)
            if gen_h != normalized and gen_h not in candidates:
                candidates.append(gen_h)

            gen_hbar = re.sub(r'([aeiou])j([uai])', r'\1ħ\2', normalized)
            if gen_hbar != normalized and gen_hbar not in candidates:
                candidates.append(gen_hbar)

        for candidate in candidates:
            if (
                candidate in self.dictionary_set
                or (hasattr(self, "suffix_generator") and bool(self.suffix_generator.has_suffix_parse(candidate)))
            ):
                return candidate
    ...
    def _short_missing_guttural_vowel_variants(self, word: str) -> list[str]:
        """Restore bounded C...VC...C losses such as xar -> xahar.

        This is intentionally evidence-only and dictionary/generated-gated.  It
        proposes inserting h/għ, optionally followed by the preceding vowel,
        between a vowel and a following consonant in short consonant-framed
        words.  The selector still applies the normal hard guards afterwards.
        """
        normalized = self._normalize_word(word)
        graphemes = list(self._graphemes(normalized))
        if len(graphemes) < 3 or len(graphemes) > 7:
            return []
        if graphemes[0] in self.VOWELS or graphemes[-1] in self.VOWELS:
            return []
        if any(token in {"h", "ħ", "għ"} for token in graphemes):
            return []
        ...
        for index in range(1, len(graphemes)):
            previous = graphemes[index - 1]
            current = graphemes[index]
            if previous not in self.VOWELS or current in self.VOWELS:
                continue
            for guttural in ("h", "għ"):
                add(graphemes[:index] + [guttural] + graphemes[index:])
                add(graphemes[:index] + [guttural, previous] + graphemes[index:])
    ...
    def _missing_h_verb_repair(self, word: str) -> str | None:
        ...
        direct_repair = self._missing_h_verb_repairs.get(normalized)
        if direct_repair:
            return direct_repair

        suffix_generator = getattr(self, "suffix_generator", None)
        if suffix_generator is not None and hasattr(suffix_generator, "parse_suffix"):
            parsed = suffix_generator.parse_suffix(normalized)
            if parsed and parsed.get("stem"):
                stem_repair = self._missing_h_verb_repairs.get(parsed["stem"])
                if stem_repair:
                    return stem_repair + parsed.get("suffix", "")

        return None

    def _invalid_ji_prefix_repair(self, word: str) -> str | None:
        """Systemically repair invalid ji- prefix on verbs taking j- or i- (e.g. jiparla -> jparla / iparla)."""
        normalized = self._normalize_word(word)
        if normalized.startswith("ji") and len(normalized) > 3:
            stem = normalized[2:]
            for candidate in (f"j{stem}", f"i{stem}", stem):
                if candidate in self.dictionary_set or self._is_verb_tagged_word(candidate):
                    return candidate
        return None

    def _has_article_prefix(self, word: str) -> bool:
        w = word.lower()
        return any(
            w.startswith(prefix)
            for prefix in (
    ...
    def _strip_article_prefix(self, word: str) -> str:
        w = word
        w_lower = w.lower()
        for prefix in (
            "il-", "l-", "iċ-", "id-", "in-", "ir-", "is-", "it-", "ix-", "iż-",
    ...
    def correct_word(self, word: str) -> str:
        normalized = self._normalize_word(word)
        if normalized in self.MANUAL_WORD_REPAIRS:
            manual_target = self.MANUAL_WORD_REPAIRS[normalized]
            return self._store_correct_word_result(
                word,
                self._match_capitalisation(word, manual_target),
                is_deterministic=True,
                candidates=(manual_target,),
            )
        cached = self._cached_correct_word(word)
        if cached is not None:
            return cached
        override_candidates = self.EXACT_SUGGESTION_OVERRIDES.get(normalized)
        if override_candidates:
            best_candidate = override_candidates[0]
            return self._store_correct_word_result(
                word,
                self._match_capitalisation(word, best_candidate),
                is_deterministic=True,
                candidates=override_candidates,
            )
        if normalized.endswith("ijja"):
            fixed_ija = normalized[:-4] + "ija"
            if fixed_ija in self.dictionary_set or True:
                return self._store_correct_word_result(
                    word,
                    self._match_capitalisation(word, fixed_ija),
                    is_deterministic=True,
                    candidates=(fixed_ija,),
                )
        negative_copula = self._negative_copula_surface_repair(normalized)
        if negative_copula and negative_copula != normalized:
            return self._store_correct_word_result(
                word,
                self._match_capitalisation(word, negative_copula),
                is_deterministic=True,
                candidates=(negative_copula,),
            )
        if normalized not in self.dictionary_set:
            terminal_apostrophe = self._strict_terminal_apostrophe_match(normalized)
            if terminal_apostrophe:
                return self._store_correct_word_result(
                    word,
                    self._match_capitalisation(word, terminal_apostrophe),
                    is_deterministic=True,
                    candidates=(terminal_apostrophe,),
                )
            if (
                "gh" in normalized
                and hasattr(self, "orthographic_generator")
                and hasattr(self.orthographic_generator, "correct_gh_priority")
            ):
                gh_shortcut = self.orthographic_generator.correct_gh_priority(
                    normalized
                )
                if gh_shortcut:
                    return self._store_correct_word_result(
                        word,
                        self._match_capitalisation(word, gh_shortcut),
                        is_deterministic=True,
                        candidates=(gh_shortcut,),
                    )
        if normalized not in self.dictionary_set:
            shortcut_matches = (
                self.orthographic_generator.dictionary_shortcut_variants(normalized)
            ...
            if len(shortcut_matches) == 1:
                shortcut = shortcut_matches[0]
                return self._store_correct_word_result(
                    word,
                    self._match_capitalisation(word, shortcut),
                    is_deterministic=True,
                    candidates=(shortcut,),
                )
        if "-" in normalized:
            ...
            if len(parts) == 2 and parts[0] and parts[1]:
                p_norm = self._normalize_word(parts[0])
                t_norm = self._normalize_word(parts[1])
                if p_norm in {
                    "il", "l", "iċ", "ic", "id", "iġ", "ig", "in", "ir", "is", "it", "ix", "iz", "iż",
                    "lil", "lill", "lir", "lit", "lid", "lin", "liċ", "lic", "liġ", "lig", "lis", "liż", "liz", "lix",
                    "tal", "tar", "tat", "tad", "tan", "taċ", "tac", "taġ", "tag", "tas", "taż", "taz", "tax",
                    "fil", "fir", "fit", "fid", "fin", "fiċ", "fic", "fiġ", "fig", "fis", "fiż", "fiz", "fix",
                    "bil", "bir", "bit", "bid", "bin", "biċ", "bic", "biġ", "big", "bis", "biż", "biz", "bix",
                    "mal", "mar", "mat", "mad", "man", "maċ", "mac", "maġ", "mag", "mas", "maż", "maz", "max",
                    "mill", "mir", "mit", "mid", "min", "miċ", "mic", "miġ", "mig", "mis", "miż", "miz", "mix",
                    "għall", "ghall", "bħall", "bhall", "sal"
                }:
                    if t_norm in self.MANUAL_WORD_REPAIRS or t_norm not in self.dictionary_set:
                        corrected_tail = self.correct_word(parts[1])
                        if corrected_tail != parts[1]:
                            res = f"{parts[0]}-{corrected_tail}"
                            lexicalized_res = self._lexicalized_form_variants(res)
                            if lexicalized_res:
                                res = lexicalized_res[0]
                            return self._store_correct_word_result(
                                word,
                                res,
                                is_deterministic=True,
        ...
        for local_prefix in ("l-", "il-", "'l-"):
            if normalized.startswith(local_prefix):
                ...
                if local_tail in {"hawn", "hemm", "hinn", "hawnhekk", "hemmhekk"}:
                    return self._store_correct_word_result(
                        word,
                        self._match_capitalisation(word, f"'l {local_tail}"),
                        is_deterministic=True,
                    )
        if (
            normalized.startswith("l'")
            and len(normalized) > 2
            and self._supports_l_apostrophe_tail(normalized[2:])
        ):
            return self._store_correct_word_result(
                word,
                word,
        ...
        initial_i_surface = self._initial_i_surface_repair(normalized)
        collapsed_glide = self._collapse_invalid_glide_doubling(normalized)
        requires_structural_review = bool(
            self._preferred_ido_l_surface(normalized)
            or self._guttural_vowel_metathesis_suffix_repairs(normalized)
            or self._pronominal_suffix_glide_variants(normalized)
            or initial_i_surface
            or collapsed_glide
            or normalized in {"xikun", "x'ikun"}
        )
        if (
            normalized in self.dictionary_set
            or self._is_recognized_surface(normalized)
        ) and not requires_structural_review:
            return self._store_correct_word_result(
                word,
                word,
                is_deterministic=True,
        ...
        if initial_i_surface:
            return self._store_correct_word_result(
                word,
                self._match_capitalisation(word, initial_i_surface),
                is_deterministic=True,
                candidates=(initial_i_surface, word),
            )
        fixed_time = self._fixed_time_expression_word(normalized)
        if fixed_time is not None:
            return self._store_correct_word_result(
        ...
        if collapsed_glide:
            return self._store_correct_word_result(
                word,
                self._match_capitalisation(word, collapsed_glide),
                is_deterministic=True,
                candidates=(collapsed_glide, word),
            )
        suffix_generator = getattr(self, "suffix_generator", None)
        if (
            suffix_generator is not None
    ...
    def _correct_word_uncached(self, word: str) -> str:
        ...
        suffix_gliding_repair = self._pronominal_suffix_gliding_repair(normalized)
        if suffix_gliding_repair:
            corrected = self._match_capitalisation(word, suffix_gliding_repair)
            return self._store_correct_word_result(
                word,
                corrected,
                is_deterministic=True,
        ...
        # Typed gh is an explicit shortcut for għ. Resolve it before the
        # broader shortcut-letter pass can promote unrelated h/doubling forms.
        if (
            "gh" in normalized
        ...
        # Kept as a fallback for unusual orthographic-generator capabilities.
        if (
            "gh" in normalized
            and orthographic_generator is not None
            and hasattr(orthographic_generator, "correct_gh_priority")
        ...
        missing_medial_guttural = self._missing_medial_guttural_variants(normalized)
        if missing_medial_guttural:
            corrected = self._match_capitalisation(word, missing_medial_guttural[0])
            return self._store_correct_word_result(
                word,
                corrected,
                is_deterministic=True,
        ...
        collapsed_glide = self._collapse_invalid_glide_doubling(normalized)
        if collapsed_glide:
            for candidate in self._suffix_repair_variants(collapsed_glide):
                if self._valid_suffix_surface_candidates(candidate):
                    corrected = self._match_capitalisation(word, candidate)
                    return self._store_correct_word_result(
                        word,
                        corrected,
                        is_deterministic=True,
        ...
        ) and self._exact_suffix_matches_cached(word):
            return self._store_correct_word_result(
                word,
                word,
                is_deterministic=True,
        ...
        if hasattr(self, "suffix_generator"):
            ...
            generated_suffix = self._correct_suffix_cached(word)
    ...
    def suggest(
        ...
        if analysis is not None and analysis.is_deterministic:
            if self._is_initial_capitalized(word):
                ...
                if (
                    corrected_place
                    and analysis.corrected
                    and self._normalize_word(corrected_place)
                    == self._normalize_word(analysis.corrected)
                ):
            ...
            if analysis.corrected:
                corrected_display = self._match_capitalisation(
                    word,
                    self._normalize_word(analysis.corrected),
                )
                deterministic = [
                    ...
                    *[
                        suggestion
                        for suggestion in deterministic
                        if self._normalize_word(suggestion)
                        != self._normalize_word(corrected_display)
                    ],
                ]
            if deterministic:
        ...
        early_structural_candidates = (
            self._short_missing_guttural_vowel_variants(normalized)
            + self._medial_guttural_vowel_restoration_variants(normalized)
            + self._restore_direct_object_h_vowel(normalized)
            + self._restore_direct_object_guttural_suffix(normalized)
        )
        hollow_ju_early = self._hollow_ju_suffix_vowel_repair(normalized)
        if hollow_ju_early:
            early_structural_candidates.append(hollow_ju_early)
        trusted_generated.update(
            self._normalize_word(candidate) for candidate in early_structural_candidates
        )
        for candidate in early_structural_candidates:
        ...
        if hasattr(self, "suffix_generator"):
            ...
            if liex_lix_match:
                ...
                generated_from_lix = self._correct_suffix_cached(liex_lix_match)
            ...
            if self._exact_suffix_matches_cached(word):
                add_generated(normalized)
                return suggestions[:limit]
            ...
            if generated_suffix:
                ...
                if hasattr(self.suffix_generator, "suggest_suffixes"):
                    for suffix_suggestion in self._suggest_suffixes_cached(
                        word,
                        limit=limit,
                    ):
        ...
        candidate_pool = self._build_candidate_evidence_pool(
            normalized,
            lexicalized_forms=lexicalized_forms,
        )
        evidence_by_word = candidate_pool.by_word()
        candidates_to_score = self._bounded_evidence_words_for_request(
            evidence_by_word,
            bulk_mode=bool(getattr(self._local, "bulk_mode", False)),
        )
        profiler = current_profiler()
        if profiler is not None:
            profiler.increment("candidate_pool_size", len(candidates_to_score))
            if len(candidates_to_score) < len(evidence_by_word):
                profiler.increment(
                    "candidate_pool_bulk_trimmed",
                    len(evidence_by_word) - len(candidates_to_score),
                )
            if any(
                source.startswith("symspell")
                for evidence in evidence_by_word.values()
                for source in evidence.sources
            ):
                profiler.increment("symspell_candidate_tokens", 1)
        ...
        guarded_rows = [
            row
            for row in rows
            if self._candidate_passes_hard_guards(
                normalized,
                evidence_by_word[row.candidate],
                row,
                max_distance=max_distance,
                score_limit=0.58,
            )
        ...
        guarded_rows.sort(
            key=lambda row: self._evidence_rank_key(
                row,
                evidence_by_word[row.candidate],
            )
        )
    ...
    def _get_meaning_index(self):
        idx = getattr(self, "meaning_index", None) or getattr(self, "_meaning_index", None)
        return idx if idx is not None else _DUMMY_MEANING_INDEX

    def _tag_meanings_for_word(self, word: str) -> list[str]:
        normalized = self._normalize_word(word)
        meanings: list[str] = []
        for tag in self.word_tags.get(normalized, ()):
    ...
    def meaning_for(self, word: str) -> str:
        ...
        if direct_object_base:
            meanings = self._get_meaning_index().meanings_for(direct_object_base)
            if meanings:
        ...
        if normalized in {"kif", "kief"}:
            meanings = self._get_meaning_index().meanings_for(normalized)
            if meanings:
                seen: list[str] = []
        ...
        if analytic_base:
            ...
            base_meaning = self._get_meaning_index().meaning_for(analytic_base)
            if base_meaning:
                return base_meaning
        ...
        if normalized.startswith("l-") and len(normalized) > 2:
            ...
            if self._is_adjective_tagged_word(superlative_tail):
                adjective_meaning = self._get_meaning_index().meaning_for(superlative_tail)
                if not adjective_meaning:
                    adjective_meaning = " / ".join(
                        self._tag_meanings_for_word(superlative_tail)
                    )
                article_rules = getattr(self, "article_phrase_rules", None)
        ...
        if normalized.startswith("'l-") and len(normalized) > 3:
            target_meaning = self.meaning_for(f"l-{normalized[3:]}")
            if target_meaning:
                return f"to {target_meaning}"
        ...
        if normalized.startswith("l'") and len(normalized) > 2:
            ...
            if self._is_verb_tagged_word(lookup_tail):
                ...
                relative_meaning = self._get_meaning_index().meaning_for(lookup_tail)
        ...
        tag_meanings = self._tag_meanings_for_word(normalized)
        if tag_meanings:
            return " / ".join(tag_meanings)

        direct_meaning = self._get_meaning_index().meaning_for(word)
        if direct_meaning:
        ...
        for candidate in self._valid_suffix_surface_candidates(word):
            meaning = format_suffix_candidate_meaning(
                candidate,
                fallback_gloss=self._get_meaning_index().meaning_for(candidate.base),
            )
    ...
    def _ambiguity_choices_uncached(
        ...
        if (
            corrected_norm != original_norm
            and
            corrected_norm in self.dictionary_set
            and self._is_verb_tagged_word(corrected_norm)
            and (
                corrected_norm in {
                    self._normalize_word(candidate)
                    for candidate in self._strict_lookup_variants(original_norm)
                }
                or self._is_direct_dictionary_shortcut(
                    original_norm,
                    corrected_norm,
                )
            )
        ...
        direct_object_licensing_records = [
            record
            for record in self._verb_records_for_surface(corrected_norm)
            if getattr(record, "person", "") in {"3SM", "2S"}
        ]
        if (
            corrected_norm.endswith("a")
            and direct_object_licensing_records
        ):
            direct_object_surface = f"{corrected_norm}ha"
            if any(
                getattr(candidate, "suffix_kind", "") == "DO"
                and getattr(candidate, "suffix_person", "") == "3SF"
                for candidate in self._valid_suffix_surface_candidates(
                    direct_object_surface
                )
            ):
                add(direct_object_surface)
        ...
        if (
            corrected_norm == original_norm
            and original_norm in self.EXACT_SUGGESTION_OVERRIDES
        ):
        ...
        if corrected_norm.startswith("l-") and len(corrected_norm) > 2:
            relative_tail = corrected_norm[2:]
            if self._is_li_relative_adjective_allowed(relative_tail):
                add(f"l'{relative_tail}")
                add(f"'l-{relative_tail}")
                return self._finalize_ambiguity_choices(
                    original_word,
                    corrected_norm,
        ...
        if (
            corrected_norm == original_norm
            and self._is_recognized_surface(corrected_norm)
            and original_norm not in self.MANUAL_WORD_SUGGESTIONS
            and not (
                hasattr(self, "orthographic_generator")
                and self.orthographic_generator.dictionary_shortcut_variants(
                    original_norm
                )
            )
        ):
        ...
        # Exact dictionary shortcut variants are stronger evidence than loose
        # homographs.  Rank them first so a missing Maltese character (for
        # example, ``niehdu`` / ``nieħdu``) cannot be displaced by an
        # unrelated doubled-initial candidate.
        if hasattr(self, "orthographic_generator"):
            ...
            for alternative in shortcut_alternatives:
                corrected_meaning = self.meaning_for(corrected_norm)
                alternative_meaning = self.meaning_for(alternative)
                if (
                    corrected_norm == original_norm
                    and corrected_meaning
                    and corrected_meaning == alternative_meaning
                ):
                    continue
                add(alternative)
                if len(ordered) >= limit:
                    break
            if shortcut_alternatives:
                return self._finalize_ambiguity_choices(
                    original_word,
                    corrected_norm,
                    ordered,
    ...
    def _finalize_ambiguity_choices(
        ...
        *,
        ...
        explicit_cross_pos = (
            self._normalize_word(original_word) in self.MANUAL_WORD_SUGGESTIONS
            or self._normalize_word(original_word) in self.EXACT_SUGGESTION_OVERRIDES
        )
        ...
        for word in ordered:
            ...
            is_fixed_time_alternative = any(
                displayed_norm in alternatives
                for alternatives in self.FIXED_TIME_ALTERNATIVES.values()
            )
            if is_fixed_time_alternative and not meaning:
                meaning = self.meaning_for(corrected_norm)

            if not displayed_norm or displayed_norm in seen_norms:
                continue
            is_literal_l_article = bool(
                displayed_norm.startswith("'l-")
                and self._is_li_relative_adjective_allowed(displayed_norm[3:])
            )
            if (
                not self._is_recognized_surface(displayed_norm)
                and not is_literal_l_article
                and not is_fixed_time_alternative
            ):
                continue
            if (
                displayed_norm != corrected_norm
                and not explicit_cross_pos
                and not self._suggestion_pos_compatible(
                    corrected_norm,
                    displayed_norm,
                )
            ):
            ...
            if (
                choices
                and displayed_canonical == corrected_canonical
                and not is_fixed_time_alternative
            ):
                continue
            if displayed_canonical in seen_canonical and not is_fixed_time_alternative:
                continue
    ...
    def _clean_visible_choices_for_token(self, token: dict) -> list[dict]:
        choices = token.get("choices", []) if isinstance(token, dict) else []
        if not choices:
            original_text = str(token.get("original", ""))
            original_norm = self._normalize_word(original_text)
            prefix = original_norm.split("-", 1)[0] if "-" in original_norm else ""
            article_rules = getattr(self, "article_phrase_rules", None)
            if prefix in {"il", "l"} and article_rules is not None:
                article_match = article_rules.match_hyphenated_article_after(
                    original_text,
                    previous=None,
                )
                if article_match is not None:
                    corrected_text = str(token.get("corrected", ""))
                    choices = list(article_match.choices)
                    if corrected_text[:1].isupper():
                        choices = [
                            {
                                **choice,
        ...
        original_norm = self._normalize_word(str(token.get("original", "")))
        corrected_norm = self._normalize_word(str(token.get("corrected", "")))
        original_text = str(token.get("original", ""))
        typed_apostrophe = "'" in original_text or "’" in original_text or "‘" in original_text
        typed_l_ambiguous = original_norm in {"l", "l-"} or original_norm.startswith(("l ", "l-"))
        typed_definite_article = bool(
            re.match(r"^(?:il|iċ|ic|id|in|ir|is|it|ix|iż|iz)-", original_norm)
        )
        corrected_3p_suffix = next(
            (
                suffix
                for suffix in ("hom", "kom")
                if corrected_norm.endswith(suffix)
            ),
            "",
        )
        explicit_choice_family = (
            original_norm in self.MANUAL_WORD_SUGGESTIONS
            or original_norm in self.EXACT_SUGGESTION_OVERRIDES
        )
        deterministic_missing_h = corrected_norm in self._missing_pronominal_h_variants(
            original_norm
        )
        contextual_i_suffix = bool(
            original_norm.startswith("i")
            and corrected_norm.startswith("i")
            and self._exact_suffix_matches_cached(corrected_norm[1:])
        )
        ...
        for choice in choices:
            if not isinstance(choice, dict):
                continue
            choice = dict(choice)
            word = str(choice.get("word", ""))
            word_norm = self._normalize_word(word)
            if not word_norm:
                continue
            if choice.get("suggestion_kind") == "family_l" and word_norm.startswith(
                ("l-", "il-")
            ):
                continue
            if (
                typed_definite_article
                and not typed_apostrophe
                and not typed_l_ambiguous
                and not original_norm.startswith(("il-", "l-"))
            ):
                if word_norm.startswith(("'l ", "'l-", "'il ", "'il-", "l'")):
                    continue
            if (
                not typed_apostrophe
                and word_norm.startswith("l'")
                and corrected_norm
                in {
                    self._normalize_word(candidate)
                    for candidate in self.orthographic_generator.remove_extra_double_variants(
                        original_norm
                    )
                }
            ):
                continue
            if original_norm.startswith(("ta ", "ta-")) and word_norm == "ta":
                continue
            if (
                corrected_3p_suffix
                and word_norm != corrected_norm
                and not word_norm.endswith(corrected_3p_suffix)
                and not explicit_choice_family
                and not choice.get("suggestion_kind")
            ):
                continue
            if deterministic_missing_h and word_norm != corrected_norm:
                continue
            if contextual_i_suffix and word_norm != corrected_norm:
                continue
            meaning = str(choice.get("meaning", "")).strip()
            if not meaning:
                meaning = self.meaning_for(word)
                if meaning:
                    choice["meaning"] = meaning
            if (
                word_norm != corrected_norm
                and not meaning
                and not choice.get("suggestion_kind")
            ):
                continue
            if (
                original_norm == corrected_norm
                and word_norm != corrected_norm
                and meaning
                and meaning == self.meaning_for(corrected_norm)
                and not explicit_choice_family
            ):
                continue
            if word_norm != corrected_norm and any(
                word_norm.replace(f"għ{vowel}", f"{vowel}għ", 1)
                == corrected_norm
                for vowel in "aeiou"
            ):
                continue
            is_literal_article_variant = bool(
                choice.get("suggestion_kind") == "family_l"
                or
                (
                    original_norm.startswith(("il ", "il-", "l ", "l-"))
                    and word_norm.startswith(("'l ", "'l-", "'il ", "'il-"))
                )
                or (
                    original_norm.startswith(("ta l ", "ta' l "))
                    and word_norm.startswith(("ta 'l ", "ta' l-"))
                )
            )
            is_fixed_time_alternative = any(
                word_norm in alternatives
                for alternatives in self.FIXED_TIME_ALTERNATIVES.values()
            )
            canonical = (
                word_norm
                if is_literal_article_variant or is_fixed_time_alternative
                else self._canonical_suggestion_key(word_norm)
            )
        ...
        visible_norms = {
            self._normalize_word(str(choice.get("word", "")))
            for choice in cleaned
            if isinstance(choice, dict)
        }
        if not visible_norms or visible_norms == {corrected_norm}:
            return []
        return cleaned
    ...
    def _bulk_ambiguity_choices(
        ...
        *,
        ...
        if original_norm == corrected_norm:
            ...
            shortcut_alternatives: list[str] = []
            if hasattr(self, "orthographic_generator"):
                corrected_meaning = self.meaning_for(corrected_norm)
                for alternative in (
                    self.orthographic_generator.dictionary_shortcut_variants(
                        original_norm
                    )
                ):
                    alternative_meaning = self.meaning_for(alternative)
                    if (
                        ...
                        continue
                    shortcut_alternatives.append(alternative)
            alternatives = [*overrides, *shortcut_alternatives]
            if not alternatives:
                return []
            return self._finalize_ambiguity_choices(
                original_word,
                corrected_norm,
                [corrected_norm, *alternatives],
                limit=limit,
            )
        ...
        if corrected_norm.endswith("a") and direct_object_licensing_records:
            direct_object_surface = f"{corrected_norm}ha"
            if any(
                getattr(candidate, "suffix_kind", "") == "DO"
                and getattr(candidate, "suffix_person", "") == "3SF"
                ...
                ordered.append(direct_object_surface)
    ...
    def correct_text_rich(self, text: str, edit_distance_tolerance: int = 1) -> dict:
        ...
        markdown_parts = re.split(r"(^[ \t]*\*\*.*?\*\*[ \t]*$)", text, flags=re.MULTILINE)
        if len(markdown_parts) > 1:
            ...
            for part in markdown_parts:
                ...
                if re.fullmatch(r"[ \t]*\*\*.*?\*\*[ \t]*", part):
                    corrected_sections.append(part)
                    all_tokens.append({"type": "text", "text": part})
                    continue
                res = self.correct_text_rich(part, edit_distance_tolerance=edit_distance_tolerance)
                corrected_sections.append(res["corrected_text"])
                all_tokens.extend(res.get("tokens", []))
            return {
                "corrected_text": "".join(corrected_sections),
                "tokens": all_tokens,
            }
        ...
        text = self._normalize_currency_amounts(repair_mojibake_text(text))
        ...
        if bulk_mode and ENABLE_BULK_TOKEN_PREWARM:
            prewarm_started = time.perf_counter()
            normalized_counts: dict[str, int] = {}
            for match in matches:
                if getattr(match, "is_quote", False):
                    continue
                normalized_surface = self._normalize_word(match.group(0))
                if normalized_surface:
                    normalized_counts[normalized_surface] = (
                        normalized_counts.get(normalized_surface, 0) + 1
                    )
            seen_prewarm: set[str] = set()
            prewarmed = 0
            for match in matches:
                if getattr(match, "is_quote", False):
                    continue
                surface = match.group(0)
                normalized_surface = self._normalize_word(surface)
                if (
                    not normalized_surface
                    or normalized_surface in seen_prewarm
                    or normalized_counts.get(normalized_surface, 0) < 2
                    or len(normalized_surface) > MAX_WORD_LENGTH
                ):
                    continue
                seen_prewarm.add(normalized_surface)
                self.correct_word(surface)
                prewarmed += 1
            profiler = current_profiler()
            if profiler is not None:
                profiler.increment("bulk_tokens_prewarmed", prewarmed)
                profiler.log_stage(
                    "bulk_token_prewarm",
                    (time.perf_counter() - prewarm_started) * 1000,
                    unique_tokens=prewarmed,
                )
        ...
        def negative_clause_has_blocker(start_index: int) -> bool:
            blockers = {"ħadd", "imkien", "qatt", "ebda", "xejn"}
            for lookahead in range(start_index, len(matches)):
                if lookahead > start_index:
                    gap = text[matches[lookahead - 1].end() : matches[lookahead].start()]
                    if re.search(r"[.!?;,\n]", gap):
                        break
                if self._normalize_word(matches[lookahead].group(0)) in blockers:
                    return True
            return False

        def negative_clause_has_previous_blocker(token_index: int) -> bool:
            blockers = {
                "ħadd", "hadd", "imkien", "qatt", "ebda", "xejn",
                "kull", "kul", "kulma", "bħalma", "bhalma",
            }
            previous_candidates = {
                self._normalize_word(previous_surface_word or ""),
            }
            if token_index > 0:
                previous_candidates.add(
                    self._normalize_word(matches[token_index - 1].group(0))
                )
            return any(candidate in blockers for candidate in previous_candidates)

        def belongs_to_capitalized_entity_run(token_index: int) -> bool:
            word = matches[token_index].group(0)
            normalized = self._normalize_word(word)
            if (
                not self._is_initial_capitalized(word)
                or normalized in self.dictionary_set
                or self._accepted_exact_english(word)
                or self._capitalized_name_kind(word)
                or self._capitalized_name_repair(word)
                or self._exact_place_word(word)
            ):
                return False

            for neighbour_index in (token_index - 1, token_index + 1):
                if not (0 <= neighbour_index < len(matches)):
                    continue
                left = min(token_index, neighbour_index)
                right = max(token_index, neighbour_index)
                gap = text[matches[left].end() : matches[right].start()]
                if not gap or not gap.isspace():
                    continue
                neighbour = matches[neighbour_index].group(0)
                neighbour_norm = self._normalize_word(neighbour)
                if (
                    self._is_initial_capitalized(neighbour)
                    and (
                        neighbour_norm not in self.dictionary_set
                        or self._capitalized_name_kind(neighbour)
                        or self._exact_place_word(neighbour)
                    )
                ):
                    return True
            return False

        def contextual_hija_is_kinship(token_index: int) -> bool:
            if token_index + 2 >= len(matches):
                return False
            if self._normalize_word(matches[token_index].group(0)) != "hija":
                return False
            if self._normalize_word(matches[token_index + 1].group(0)) != "ma":
                return False
            verb_surface = self.correct_word(matches[token_index + 2].group(0))
            records = self._verb_records_for_surface(verb_surface)
            persons = {getattr(record, "person", "") for record in records}
            if "3SM" in persons and "3SF" not in persons:
                return True
            normalized_verb = self._normalize_word(verb_surface)
            return bool(
                self._ta_direct_object_gloss(normalized_verb)
                and normalized_verb.startswith(("j", "i"))
                and not normalized_verb.startswith(("itt",))
            )
        ...
        while index < len(matches):
            ...
            if getattr(match, "is_quote", False):
                ...
                canonical_english, _quote_is_recognized = self._correct_quoted_english_text(
                    inner_text
                )
                tokens.append(
                    {
            ...
            cultural_name_repair = self._capitalized_name_repair(original_word)
            if (
                cultural_name_repair
                and self._normalize_word(cultural_name_repair)
                in self._maltese_cultural_name_set()
            ):
                tokens.append(
                    {
                        "type": "word",
                        "original": original_word,
                        "corrected": cultural_name_repair,
                        "meaning": "",
                        "ambiguous": False,
                        "choices": [],
                        "name_like": True,
                ...
                corrected_parts.append(cultural_name_repair)
                previous_surface_word = self._normalize_word(cultural_name_repair)
                last_end = match.end()
                index += 1
                continue

            if index + 1 < len(matches) and article_rules is not None:
                next_word = matches[index + 1].group(0)
                epenthetic_next = self._epenthetic_place_surface(
                    next_word,
                    prefer_initial_i=True,
                )
                epenthetic_place_prefixes = {
                    "il", "l", "fi", "fil", "fl", "bi", "bil", "bl",
                    "ma", "ma'", "ta", "ta'", "min", "minn",
                    "għal", "ghal", "għall", "ghall",
                    "bħal", "bhal", "bħall", "bhall",
                }
                if epenthetic_next and original_norm in epenthetic_place_prefixes:
                    corrected_phrase = article_rules.preposition_article_form(
                        original_norm,
                        next_word,
                    )
                    if corrected_phrase:
                        ...
                        is_ambiguous, is_crucial = token_choice_state(phrase_choices)
                        tokens.append(
                            {
                                "type": "phrase",
                                "original": text[
                        ...
                        previous_surface_word = self._normalize_word(epenthetic_next)
                        last_end = matches[index + 1].end()
                        index += 2
                        continue

            if original_norm == "ta" and index + 1 < len(matches):
                next_word = matches[index + 1].group(0)
                next_norm = self._normalize_word(next_word)
                corrected_adverb = self.correct_word(next_word)
                corrected_adverb_norm = self._normalize_word(corrected_adverb)
                if next_norm.startswith("i") and len(next_norm) > 1:
                    corrected_tail = self.correct_word(next_norm[1:])
                    if self._is_adverb_tagged_word(corrected_tail):
                        corrected_adverb = self._match_capitalisation(
                            next_word,
                            f"i{self._normalize_word(corrected_tail)}",
                        )
                        corrected_adverb_norm = self._normalize_word(corrected_adverb)
                if (
                    self._is_adverb_tagged_word(corrected_adverb_norm)
                    or (
                        corrected_adverb_norm.startswith("i")
                        and self._is_adverb_tagged_word(corrected_adverb_norm[1:])
                    )
                ):
                    ta_decision = self._z_ta_phrase_decision(
                        original_word=original_word,
                        corrected_next=corrected_adverb,
                        sentence_initial=sentence_initial,
                    )
                    is_ambiguous, is_crucial = token_choice_state(
                        ta_decision.choices,
                        force_crucial=ta_decision.force_crucial,
                    )
                    tokens.append(
                        {
                            "type": "phrase",
                            ...
                            "corrected": ta_decision.corrected_phrase,
                            "ambiguous": is_ambiguous,
                            "crucial": is_crucial,
                            "choices": ta_decision.choices,
                            "name_like": False,
                        }
                    )
                    corrected_parts.append(ta_decision.corrected_phrase)
                    previous_surface_word = ta_decision.previous_surface
                    last_end = matches[index + 1].end()
                    index += 2
                    continue

            usage_suggestions = self._maltese_usage_suggestions(original_word)
            if usage_suggestions:
                corrected_usage = (
                    self._capitalize_first_letter(original_word)
                    if sentence_initial
                    else original_word
                )
                surfaced_suggestions = [
                    self._apply_surface_case(
                        original_word,
                        self._surface_maltese_usage_suggestion(
                            suggestion,
                ...
                tokens.append(
                    {
                        "type": "maltese_usage",
                        "original": original_word,
                        "corrected": corrected_usage,
                        "inner_text": original_word,
                        "maltese_suggestion": surfaced_suggestions,
                    }
                )
                corrected_parts.append(corrected_usage)
                previous_surface_word = self._normalize_word(corrected_usage)
                last_end = match.end()
                index += 1
                continue

            epenthetic_place = self._epenthetic_place_surface(
                original_word,
                prefer_initial_i=prefer_initial_vowel_surface,
            )
            if epenthetic_place is not None:
                tokens.append(
                    {
                        "type": "word",
                        "original": original_word,
                        "corrected": epenthetic_place,
                        "meaning": self.meaning_for(
                            self._epenthetic_place_base(original_word) or original_word
                        ),
                        "ambiguous": False,
                        "choices": [],
                        "name_like": True,
                ...
                corrected_parts.append(epenthetic_place)
                previous_surface_word = self._normalize_word(epenthetic_place)
                last_end = match.end()
                index += 1
                continue

            if contextual_hija_is_kinship(index):
                corrected_hija = self._apply_surface_case(
                    original_word,
                    "ħija",
                    sentence_initial=sentence_initial,
                )
                tokens.append(
                    {
                        "type": "word",
                        "original": original_word,
                        "corrected": corrected_hija,
                        "meaning": self.meaning_for("ħija"),
                        "ambiguous": False,
                        "choices": [],
                        "name_like": False,
                    }
                )
                corrected_parts.append(corrected_hija)
                previous_surface_word = "ħija"
                last_end = match.end()
                index += 1
                continue

            if belongs_to_capitalized_entity_run(index):
                tokens.append(
                    {
                        "type": "word",
                        "original": original_word,
            ...
            if (
                ...
                corrected_parts.append(corrected_phrase)
                previous_surface_word = "minħabba"
                last_end = matches[index + 1].end()
                index += 2
                continue
            ...
            if original_norm in {"di", "din"} and index + 1 < len(matches):
                next_norm_for_dil = self._normalize_word(matches[index + 1].group(0))
                if next_norm_for_dil.startswith("l-") and len(next_norm_for_dil) > 2:
                    corrected_phrase = f"dil-{next_norm_for_dil.split('-', 1)[1]}"
                    corrected_phrase = self._apply_surface_case(
                        original_word,
                        corrected_phrase,
                        sentence_initial=sentence_initial,
            ...
            if (
                ...
                next_article_head = self._normalize_word(
                    matches[index + 1].group(0)
                )
                canonical_preposition = split_preposition_prefixes.get(
                    original_norm
                )
                if (
                    canonical_preposition
                    and next_article_head in split_article_heads
                ):
                    article_tail_word = matches[index + 2].group(0)
                    corrected_article_tail = self.correct_word(article_tail_word)
                    corrected_article_tail_norm = self._normalize_word(
                        corrected_article_tail
                    )
                    corrected_phrase = article_rules.preposition_article_form(
                        canonical_preposition,
                        corrected_article_tail_norm,
                    )
                    if corrected_phrase:
                        corrected_phrase = (
                        ...
                        tokens.append(
                            {
                                "type": "phrase",
                                "original": text[
                                    match.start() : matches[index + 2].end()
                                ],
                                "corrected": corrected_phrase,
                                "meaning": "",
                                "ambiguous": False,
                                ...
                                "choices": [],
                                "name_like": bool(
                                    self._capitalized_name_kind(article_tail_word)
                                    or self._exact_place_word(article_tail_word)
                                ),
                            }
                        )
                        corrected_parts.append(corrected_phrase)
            ...
            if index + 1 < len(matches):
                next_word_for_z_guard = matches[index + 1].group(0)
                next_norm_for_z_guard = self._normalize_word(next_word_for_z_guard)
                corrected_next_for_z_guard = self.correct_word(next_word_for_z_guard)
                corrected_next_norm_for_z_guard = self._normalize_word(
                    corrected_next_for_z_guard
                )
                protected_z_phrase = None
                protected_previous_surface = None
                protected_token_original = text[
                    match.start() : matches[index + 1].end()
                ]
                protected_token_corrected = None
                protected_choices: list[dict] = []
                if original_norm in {"ma", "ma'"} and corrected_next_norm_for_z_guard == "tantx":
                    protected_z_phrase = self._apply_surface_case(
                        original_word,
                        f"ma {corrected_next_for_z_guard}",
                        sentence_initial=sentence_initial,
                    )
                    protected_previous_surface = corrected_next_norm_for_z_guard
                elif original_norm in {"min", "minn"}:
                    min_decision = self._z_min_phrase_decision(
                        original_word=original_word,
                        current_norm=original_norm,
                        next_norm=next_norm_for_z_guard,
                        corrected_next=corrected_next_for_z_guard,
                        sentence_initial=sentence_initial,
                    )
                    if (
                        ...
                        protected_token_original = min_decision.token_original
                        protected_token_corrected = min_decision.token_corrected
                        protected_choices = min_decision.choices

                if protected_z_phrase is not None:
                    protected_ambiguous, protected_crucial = token_choice_state(
                        protected_choices,
                        force_crucial=bool(
                            original_norm in {"min", "minn"}
                            and len(protected_choices) > 1
                        ),
                    )
                    tokens.append(
                        {
                            "type": "phrase",
                            "original": protected_token_original,
                            "corrected": (
                                protected_token_corrected or protected_z_phrase
                            ),
                            "meaning": "",
                            "ambiguous": protected_ambiguous,
                            "crucial": protected_crucial,
                            "choices": protected_choices,
                            "name_like": False,
                        }
                    )
                    if (
                        protected_token_corrected
                        and protected_z_phrase.startswith(
                            protected_token_corrected
                        )
                    ):
                        protected_tail_text = protected_z_phrase[
                            len(protected_token_corrected) :
                        ]
                        if protected_tail_text:
                            tokens.append(
                                {
                                    "type": "text",
                                    "text": protected_tail_text,
                                }
                            )
                    corrected_parts.append(protected_z_phrase)
                    previous_surface_word = protected_previous_surface
                    last_end = matches[index + 1].end()
                    index += 2
                    continue
            ...
            if original_norm in {"ma", "ma'"} and index + 1 < len(matches):
                next_norm = self._normalize_word(matches[index + 1].group(0))
                if next_norm in {"l", "il"} and index + 2 < len(matches):
                    article_noun = matches[index + 2].group(0)
                    corrected_article_noun = self.correct_word(article_noun)
                    corrected_article_noun_norm = self._normalize_word(
                        corrected_article_noun
                    )
                    if corrected_article_noun_norm:
                        corrected_phrase = f"mal-{corrected_article_noun_norm}"
                        if sentence_initial:
                            corrected_phrase = self._capitalize_first_letter(
                                corrected_phrase
                            )
                        ...
                        index += 3
                        continue
                if next_norm in {"ċ", "c", "d", "n", "r", "s", "t", "x", "z", "ż"} and index + 2 < len(matches):
                    article_noun = matches[index + 2].group(0)
                    corrected_article_noun = self.correct_word(article_noun)
                    corrected_article_noun_norm = self._normalize_word(
                        corrected_article_noun
                    ...
                    if corrected_article_noun_norm:
                        prefix = "ċ" if next_norm == "c" else next_norm
                        corrected_phrase = f"ma{prefix}-{corrected_article_noun_norm}"
                        if sentence_initial:
                            corrected_phrase = self._capitalize_first_letter(
                                corrected_phrase
                            )
                ...
                corrected_next_norm = self._normalize_word(corrected_next)
                next_is_proper_name = bool(
                    self._capitalized_name_kind(next_word)
                    or self._exact_place_word(next_word)
                )
                if next_is_proper_name:
                    if corrected_next_norm.startswith(("għ", "gh", "h")) or (
                        corrected_next_norm
                        and corrected_next_norm[0] in self.VOWELS
                    ):
                        display_ma = "M'" if sentence_initial else "m'"
                        corrected_phrase = f"{display_ma}{corrected_next}"
                    else:
                        display_ma = "Ma'" if sentence_initial else "ma'"
                        corrected_phrase = f"{display_ma} {corrected_next}"
                    tokens.append(
                        {
                            "type": "phrase",
                ...
                if corrected_next_norm.startswith("il-"):
                    ...
                    article_tail = corrected_next_norm[2:]
                elif re.match(r"^[ċdnrstxżz]-", corrected_next_norm):
                    article_tail = corrected_next_norm
                if article_tail:
                    corrected_phrase = (
                        f"ma{article_tail}"
                        if "-" in article_tail and article_tail[:1] not in self.VOWELS
                        else f"mal-{article_tail}"
                    )
                    if sentence_initial:
                        corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                    tokens.append(
                ...
                if (
                    corrected_next_norm
                    and corrected_next_norm not in {"tantx"}
                    and not corrected_next_norm.startswith(("għ", "gh", "h"))
                    and not (corrected_next_norm[0] in self.VOWELS)
                    and not self._is_verb_tagged_word(corrected_next_norm)
                    and (
                        self._is_probable_noun(corrected_next_norm)
                        or self._is_recognized_surface(corrected_next_norm)
                    )
                ):
                    corrected_phrase = f"ma' {corrected_next_norm}"
                    if sentence_initial:
                        corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                    tokens.append(
                        {
            ...
            if original_norm in {"lil", "lill"} and index + 1 < len(matches):
                corrected_next = self.correct_word(matches[index + 1].group(0))
                corrected_next_norm = self._normalize_word(corrected_next)
                if self._is_pronoun_tagged_word(corrected_next_norm):
                    display_lil = self._apply_surface_case(
                        original_word,
                        "lil",
                        sentence_initial=sentence_initial,
                    )
                    corrected_phrase = f"{display_lil} {corrected_next_norm}"
                    tokens.append(
                        {
                            "type": "phrase",
                            "original": text[match.start() : matches[index + 1].end()],
            ...
            if original_norm == "li" and index + 1 < len(matches):
                ...
                if self._is_verb_tagged_word(protected_next):
                    display_li = self._apply_surface_case(
                        original_word,
                        "li",
                        sentence_initial=sentence_initial,
                    )
                    corrected_phrase = f"{display_li} {protected_next}"
                    if sentence_initial:
                        corrected_phrase = self._capitalize_first_letter(
                            corrected_phrase
                        )
            ...
            if original_norm.startswith("j"):
                ...
                if (
                    ...
                    and (
                        self._is_verb_or_pronoun_tagged(verified_verb_norm)
                        or bool(
                            self._valid_suffix_surface_candidates(
                                verified_verb_norm
                            )
                        )
                    )
            ...
            lowercase_spelling_repair = self.correct_word(original_word)
            lowercase_spelling_repair_norm = self._normalize_word(lowercase_spelling_repair)
            if (
                exact_proper_name
                ...
                and not (
                    lowercase_spelling_repair_norm
                    and lowercase_spelling_repair_norm != original_norm
                    and self._is_recognized_surface(lowercase_spelling_repair_norm)
                    and self._normalize_word(exact_proper_name)
                    not in self.country_maltese_to_english
                )
            ):
                tokens.append(
                    {
            ...
            direct_manual_repair = self.MANUAL_WORD_REPAIRS.get(original_norm)
            if direct_manual_repair:
                corrected_manual = self._apply_surface_case(
                    original_word,
                    direct_manual_repair,
                    sentence_initial=sentence_initial,
                )
                tokens.append(
                    {
                        "type": "word",
                        "original": original_word,
                        "corrected": corrected_manual,
                        "meaning": self.meaning_for(direct_manual_repair),
                        "ambiguous": False,
                        "choices": [],
                        "name_like": False,
                    }
                )
                corrected_parts.append(corrected_manual)
                previous_surface_word = self._normalize_word(corrected_manual)
                last_end = match.end()
                index += 1
                continue
            ...
            if index + 1 < len(matches) and not getattr(
                ...
                if (
                    article_rules is not None
                    and original_norm in {"bhal", "bħal", "ghal", "għal"}
                ):
                    article_prefix = (
                        "bħall" if original_norm in {"bhal", "bħal"} else "għall"
                    )
                    literal_prefix = (
                        "bħal" if article_prefix == "bħall" else "għal"
                    )
                    consumed_article_phrase = 0
                    article_tail_word = ""
                    if (
                        next_norm_for_phrase in {"l", "il"}
                        and index + 2 < len(matches)
                        and not getattr(matches[index + 2], "is_quote", False)
                        and text[match.end() : matches[index + 1].start()].isspace()
                    ):
                        consumed_article_phrase = 3
                        article_tail_word = word_tokens[index + 2].text
                    elif "-" in separator_between:
                        consumed_article_phrase = 2
                        article_tail_word = next_word_for_phrase

                    if consumed_article_phrase and article_tail_word:
                        corrected_tail = self._normalize_word(
                            self.correct_word(article_tail_word)
                        )
                        article_form = article_rules.preposition_article_form(
                            article_prefix,
                            corrected_tail,
                        ...
                        if article_form:
                            ...
                            if not phrase_choices:
                                phrase_choices = [
                                    {
                                        "word": corrected_phrase,
                                        "meaning": self.meaning_for(corrected_tail),
                                    },
                                    {
                                        "word": f"{literal_prefix} {corrected_tail}",
                                        "meaning": self.meaning_for(corrected_tail),
                                    },
                                ]
                            if sentence_initial:
                            ...
                            tokens.append(
                                {
                                    "type": "phrase",
                                    "original": text[
                                        matches[index].start() : matches[
                                            index + consumed_article_phrase - 1
                                        ].end()
                                    ],
                                    "corrected": corrected_phrase,
                                    "ambiguous": is_ambiguous,
                ...
                if (
                    (
                        ...
                        or (
                            ...
                            and (
                                not self._is_verb_tagged_word(original_norm)
                                or next_norm_for_phrase.startswith(
                                    original_norm[-1:]
                                )
                            )
                            and not any(
                                str(tag).startswith(("ADVERB", "CONJ", "PRON"))
                                for tag in self.word_tags.get(original_norm, set())
                            )
                        )
                    )
                    ...
                    and not (
                        ...
                        and article_rules._is_function_word_tail(
                    ...
                    and not (
                        article_rules._assimilated_prefix_key(
                            next_norm_for_phrase
                        )
                        in {"c", "d", "n", "r", "s", "t", "x", "z"}
                        and index + 2 < len(matches)
                    ...
                    if original_norm in {"il", "l"} and tail_is_capitalized:
                        ...
                        if self._accepted_article_english(lookup_word_for_phrase):
            ...
            if (
                early_restorative
                and self._blocks_personal_name_gh_repair(
                    original_word,
                    early_restorative[0],
                    sentence_initial=sentence_initial,
                )
            ...
            if original_norm not in self.dictionary_set:
                ...
                cw = self.correct_word(original_word)
                cw_norm = self._normalize_word(cw)
                if self._blocks_personal_name_gh_repair(
                    original_word,
                    cw,
                    sentence_initial=sentence_initial,
                ):
                    cw = original_word
                    cw_norm = original_norm
                if cw_norm != original_norm and (
                    cw_norm in self.dictionary_set
                    or self._is_recognized_surface(cw_norm)
                    or self._valid_generated_surface(cw_norm)
                ):
                    token_repairs = [cw]
                elif doubled_letter:
                    token_repairs = [doubled_letter]
                else:
                    token_repairs = []
            if token_repairs:
                corrected_word = token_repairs[0]
                ...
                choices = (
                    ...
                    if not bulk_mode
                    else self._bulk_ambiguity_choices(
                        original_word,
                        corrected_word,
                        limit=3,
                    )
                ...
                is_name_like = (
                    ...
                    and original_norm not in self.EXACT_SUGGESTION_OVERRIDES
                )
            ...
            if index + 1 < len(matches) and getattr(
                ...
                if article_prefix is None:
                    ...
                    if article_prefix is None and current_norm in ALL_FORMS:
                        ...
                        word_stem = ""
                        for s in BASE_PREPS:
                            if current_norm_check not in {"għad", "ghad"} and current_norm_check.startswith(s):
                                word_stem = s
                                break

                        MALTESE_STEMS = {"gha": "għa", "bha": "bħa"}
                        MALTESE_PREPS = {"gha": "għall", "bha": "bħall", "li": "lill"}
            ...
            if index + 1 < len(matches) and not getattr(
                ...
                norm_num = current_norm
                if norm_num.endswith("-il"):
                    norm_num = norm_num[:-3]

                if norm_num in TEEN_NUMERAL_MAP:
                    canonical_num = TEEN_NUMERAL_MAP[norm_num]
                    # Case A: followed by "il", "l", "il-", "l-"
                    if next_norm in {"il", "l", "il-", "l-"}:
                        if index + 2 < len(matches):
                            third_word = word_tokens[index + 2].text
                            stripped_noun = self._strip_article_prefix(self.correct_word(third_word))
                            corrected_phrase = f"{canonical_num}-il {stripped_noun}"
                            if sentence_initial:
                                corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                            original_phrase = text[matches[index].start() : matches[index + 2].end()]
                            tokens.append(
                                {
                                    "type": "phrase",
                                    "original": original_phrase,
                            ...
                            corrected_parts.append(corrected_phrase)
                            previous_surface_word = self._normalize_word(stripped_noun)
                            last_end = matches[index + 2].end()
                            index += 3
                            continue
                        else:
                            corrected_phrase = f"{canonical_num}-il"
                            if sentence_initial:
                                corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                            original_phrase = text[matches[index].start() : matches[index + 1].end()]
                            tokens.append(
                                {
                                    "type": "phrase",
                                    "original": original_phrase,
                            ...
                            corrected_parts.append(corrected_phrase)
                            previous_surface_word = "il"
                            last_end = matches[index + 1].end()
                            index += 2
                            continue
                    # Case B: followed by an article-prefixed noun (e.g. is-siegħa, il-siegħa, it-tifel)
                    elif self._has_article_prefix(next_norm):
                        stripped_noun = self._strip_article_prefix(self.correct_word(next_word))
                        corrected_phrase = f"{canonical_num}-il {stripped_noun}"
                        if sentence_initial:
                            corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                        original_phrase = text[matches[index].start() : matches[index + 1].end()]
                        ...
                        index += 2
                        continue
                    # Case C: token itself ends with "-il" (e.g. 11-il, ħdax-il) followed by a noun
                    elif current_norm.endswith("-il"):
                        stripped_noun = self._strip_article_prefix(self.correct_word(next_word))
                        corrected_phrase = f"{canonical_num}-il {stripped_noun}"
                        if sentence_initial:
                            corrected_phrase = self._capitalize_first_letter(corrected_phrase)
                ...
                if current_norm == "x":
                    corrected_next = self.correct_word(next_word)
                    corrected_phrase = (
                        self._capitalize_first_letter(f"x'{corrected_next}")
                        if sentence_initial
                        else f"x'{corrected_next}"
                    )
                    tokens.append(
                        {
                            "type": "phrase",
                    ...
                    corrected_parts.append(corrected_phrase)
                    previous_surface_word = corrected_next
                    last_end = matches[index + 1].end()
                    index += 2
                    continue

                if current_norm in {"kull", "kul"} and next_norm == "ma":
                    corrected_phrase = self._capitalize_first_letter("kulma") if sentence_initial else "kulma"
                    tokens.append(
                        {
                            "type": "phrase",
                            "original": text[matches[index].start() : matches[index + 1].end()],
                    ...
                    corrected_parts.append(corrected_phrase)
                    previous_surface_word = "kulma"
                    last_end = matches[index + 1].end()
                    index += 2
                    continue

                if current_norm in {"dak", "dat"} and index + 2 < len(matches):
                    w1 = self._normalize_word(text[matches[index + 1].start() : matches[index + 1].end()])
                    w2 = self._normalize_word(text[matches[index + 2].start() : matches[index + 2].end()])
                    if w1 in {"il", "l", "il-", "l-"} and w2 in {"vitamini", "vitamina"}:
                        corrected_phrase = (
                            self._capitalize_first_letter("dawk il-vitamini")
                            if sentence_initial
                            else "dawk il-vitamini"
                        )
                        tokens.append(
                            {
                                "type": "phrase",
                                "original": text[matches[index].start() : matches[index + 2].end()],
                                "corrected": corrected_phrase,
                                "ambiguous": False,
                                "crucial": True,
                                "choices": [],
                        ...
                        corrected_parts.append(corrected_phrase)
                        previous_surface_word = "vitamini"
                        last_end = matches[index + 2].end()
                        index += 3
                        continue

                if current_norm == "dak" and (next_norm == "vitamini" or next_norm == "il-vitamini"):
                    corrected_phrase = (
                        self._capitalize_first_letter("dawk il-vitamini")
                        if sentence_initial
                        else "dawk il-vitamini"
                ...
                if current_norm == "u" and (next_norm.startswith("it-") or next_norm.startswith("il-")):
                    stem = next_norm.split("-", 1)[1]
                    corrected_next_stem = self.correct_word(stem)
                    corrected_phrase = self._capitalize_first_letter(f"u t-{corrected_next_stem}") if sentence_initial else f"u t-{corrected_next_stem}"
                    tokens.append({
                        "type": "phrase",
                        "original": text[matches[index].start() : matches[index + 1].end()],
                        "corrected": corrected_phrase,
                    ...
                    corrected_parts.append(corrected_phrase)
                    previous_surface_word = corrected_next_stem
                    last_end = matches[index + 1].end()
                    index += 2
                    continue

                if current_norm in {"lill", "lil"} and next_norm == "bina":
                    corrected_phrase = self._capitalize_first_letter("lill-ibnu") if sentence_initial else "lill-ibnu"
                    tokens.append({
                        "type": "phrase",
                        "original": text[matches[index].start() : matches[index + 1].end()],
                        "corrected": corrected_phrase,
                    ...
                    corrected_parts.append(corrected_phrase)
                    previous_surface_word = "ibnu"
                    last_end = matches[index + 1].end()
                    index += 2
                    continue

                if sentence_initial and current_norm in {"'l", "'l-", "'L", "'L-"}:
                    current_norm = "l"
                if sentence_initial and current_norm == "i":
                    current_norm = "il"

                article_and_fusion_stems = {
                    "il", "l",
                    "iċ", "ic", "id", "iġ", "ig", "in", "ir", "is", "it", "ix", "iz", "iż",
                    "lil", "lill", "lir", "lit", "lid", "lin", "liċ", "lic", "liġ", "lig", "lis", "liż", "liz", "lix",
                    "tal", "tar", "tat", "tad", "tan", "taċ", "tac", "taġ", "tag", "tas", "taż", "taz", "tax",
                    "fil", "fir", "fit", "fid", "fin", "fiċ", "fic", "fiġ", "fig", "fis", "fiż", "fiz", "fix",
                    "bil", "bir", "bit", "bid", "bin", "biċ", "bic", "biġ", "big", "bis", "biż", "biz", "bix",
                    "mal", "mar", "mat", "mad", "man", "maċ", "mac", "maġ", "mag", "mas", "maż", "maz", "max",
                    "mill", "mir", "mit", "mid", "min", "miċ", "mic", "miġ", "mig", "mis", "miż", "miz", "mix",
                    "għall", "ghall", "bħall", "bhall", "sal", "mic", "miċ"
                }

                standalone_article_fragments = {
                    "ċ": "ċ", "c": "ċ",
                    "d": "d",
                    "ġ": "ġ", "g": "ġ",
                    "n": "n",
                ...
                if current_norm in standalone_article_fragments:
                    corrected_next = self.correct_word(next_word)
                    corrected_next_norm = self._normalize_word(corrected_next)
                    next_first = (
                        self._graphemes(corrected_next_norm)[0]
                        if corrected_next_norm
                        else ""
                    )
                    article_letter = standalone_article_fragments[current_norm]
                    if next_first == article_letter:
                        p_norm = self._normalize_word(previous_surface_word or "")
                        prefix = (
                            f"{article_letter}-"
                            if p_norm and p_norm[-1:] in self.VOWELS
                            else f"i{article_letter}-"
                        )
                        corrected_phrase = self._apply_surface_case(
                            original_word,
                            f"{prefix}{corrected_next}",
                            sentence_initial=sentence_initial,
                        )
                        tokens.append(
                            {
                ...
                if current_norm in {"lil", "lill"} and index + 1 < len(matches):
                    corrected_next = self.correct_word(next_word)
                    corrected_next_norm = self._normalize_word(corrected_next)
                    if self._is_pronoun_tagged_word(corrected_next_norm):
                        display_lil = self._apply_surface_case(
                ...
                if current_norm in article_and_fusion_stems and not next_norm.startswith("-") and not current_norm.endswith("-"):
                    corrected_next = self.correct_word(next_word)
                    corrected_next_norm = self._normalize_word(corrected_next)
                    if (
                        current_norm != "il"
                        and (
                            next_norm in {"il", "l", "i", *standalone_article_fragments}
                            or corrected_next_norm in {"il", "l", "i", *standalone_article_fragments}
                        )
                    ):
                        pass
                    elif current_norm == "mar" and not (next_norm.startswith("r") or corrected_next_norm.startswith("r")):
                        pass
                    elif corrected_next_norm and not any(corrected_next_norm.startswith(prefix) for prefix in (
                        "il-", "l-", "iċ-", "ic-", "id-", "iġ-", "ig-", "in-", "ir-", "is-", "it-", "ix-", "iz-", "iż-",
                        "lil-", "lill-", "lir-", "lit-", "lid-", "lin-", "liċ-", "lic-", "liġ-", "lig-", "lis-", "liż-", "liz-", "lix-",
                        "tal-", "tar-", "tat-", "tad-", "tan-", "taċ-", "tac-", "taġ-", "tag-", "tas-", "taż-", "taz-", "tax-",
                        "fil-", "fir-", "fit-", "fid-", "fin-", "fiċ-", "fic-", "fiġ-", "fig-", "fis-", "fiż-", "fiz-", "fix-",
                        ...
                        first_grapheme = self._graphemes(corrected_next_norm)[0]
                        first_ch = first_grapheme
                        ...
                        all_sun = {"ċ", "c", "d", "ġ", "g", "n", "r", "s", "t", "x", "z", "ż"}

                        if first_ch.isdigit():
                            fused_art = f"{current_norm}-"
                        elif current_norm in {"il", "l"} or (len(current_norm) == 2 and current_norm.startswith("i")):
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "l-" if current_norm == "l" else "il-"
                            elif first_ch in vowels:
                                fused_art = "l-"
                            elif first_ch in all_sun:
                                real_sun = "ġ" if first_ch in ("g", "ġ") else ("ċ" if first_ch in ("c", "ċ") else ("ż" if first_ch in ("z", "ż") else first_ch))
                                fused_art = f"i{real_sun}-"
                            else:
                                prev_ends_vowel = False
                                if previous_surface_word:
                                    p_norm = self._normalize_word(previous_surface_word)
                                    if p_norm and p_norm[-1] in vowels:
                                        prev_ends_vowel = True
                                fused_art = "l-" if prev_ends_vowel else "il-"
                        elif current_norm.startswith("ta") and len(current_norm) == 3:
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "tal-"
                            elif first_ch in all_sun:
                                real_sun = "ġ" if first_ch in ("g", "ġ") else ("ċ" if first_ch in ("c", "ċ") else ("ż" if first_ch in ("z", "ż") else first_ch))
                                fused_art = f"ta{real_sun}-"
                            else:
                                fused_art = "tal-"
                        elif current_norm.startswith("bi") and len(current_norm) == 3:
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "bil-"
                            elif first_ch in all_sun:
                                real_sun = "ġ" if first_ch in ("g", "ġ") else ("ċ" if first_ch in ("c", "ċ") else ("ż" if first_ch in ("z", "ż") else first_ch))
                                fused_art = f"bi{real_sun}-"
                            else:
                                fused_art = "bil-"
                        elif current_norm.startswith("fi") and len(current_norm) == 3:
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "fil-"
                            elif first_ch in all_sun:
                                real_sun = "ġ" if first_ch in ("g", "ġ") else ("ċ" if first_ch in ("c", "ċ") else ("ż" if first_ch in ("z", "ż") else first_ch))
                                fused_art = f"fi{real_sun}-"
                            else:
                                fused_art = "fil-"
                        elif current_norm.startswith("mi") and len(current_norm) == 3 and current_norm != "mill":
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "mill-"
                            elif first_ch in all_sun:
                                real_sun = "ġ" if first_ch in ("g", "ġ") else ("ċ" if first_ch in ("c", "ċ") else ("ż" if first_ch in ("z", "ż") else first_ch))
                                fused_art = f"mi{real_sun}-"
                            else:
                                fused_art = "mill-"
                        elif current_norm in {"lil", "lill"}:
                            if first_ch in vowels:
                                fused_art = "lill-"
                            elif first_ch in sun_letters_dict:
                                fused_art = f"{sun_letters_dict[first_ch]}-"
                            else:
                                fused_art = "lil-"
                        else:
                            fused_art = f"{current_norm}-"

                        fused_phrase = f"{fused_art}{corrected_next}"
                        if sentence_initial:
                            fused_phrase = fused_phrase.lstrip("'")
                            fused_phrase = self._capitalize_first_letter(fused_phrase)

                        tokens.append(
                            {
                                "type": "phrase",
                                "original": text[matches[index].start() : matches[index + 1].end()],
                                "corrected": fused_phrase,
                                "ambiguous": False,
                                "crucial": True,
                                "choices": [],
                            }
                        )
                        corrected_parts.append(fused_phrase)
                        previous_surface_word = corrected_next
                        last_end = matches[index + 1].end()
                        index += 2
                        continue
                ...
                elif current_norm in {"għad", "ghad"}:
                    protected_phrase = f"għad {self.correct_word(next_word)}"
            ...
            if current_norm in {"ma", "ma'"} and self._normalize_word(
                previous_surface_word or ""
            ) != "mingħajr":
                corrected_next = (
                ...
                corrected_next = self._apply_initial_vowel_surface(
                    next_word,
                    corrected_next,
                    prefer_vowel_surface=False,
                )
                corrected_next_norm = self._normalize_word(corrected_next)
                if (
                    not self._is_verb_tagged_word(next_word)
                    and len(self._letter_tokens(next_norm)) <= 4
                    and "għ" in corrected_next_norm
                    and not any(marker in next_norm for marker in ("għ", "gh", "h"))
                ):
                    corrected_next = next_word
                    corrected_next_norm = next_norm
                is_verb = self._is_verb_tagged_word(corrected_next)
                negative_imperative = (
                    self._negative_imperative_form(next_word)
                    or self._negative_imperative_form(corrected_next)
                )
                previous_negative_blocker = negative_clause_has_previous_blocker(index)
                if negative_imperative and previous_negative_blocker:
                    display_ma = self._apply_surface_case(
                        original_word,
                        "ma",
                        sentence_initial=sentence_initial,
                    )
                    corrected_phrase = f"{display_ma} {next_norm}"
                    tokens.append(
                        {
                            "type": "phrase",
                            "original": text[
                    ...
                    index += 2
                    continue
                if negative_imperative and not previous_negative_blocker:
                    ...
                    is_verb
                    and not self._normalize_word(corrected_next).endswith("x")
                    and not negative_clause_has_blocker(index + 2)
                    and not previous_negative_blocker
                ):
                    negative_surface = self._negative_verb_form(corrected_next)
                    if negative_surface is not None:
                        corrected_next = self._match_capitalisation(
                            next_word,
                            negative_surface,
                        )

                ma_decision = self._z_ma_phrase_decision(
                    original_word=original_word,
                ...
                if ma_decision is not None:
                    if sentence_initial:
                        ma_decision.corrected_phrase = self._capitalize_first_letter(
                            ma_decision.corrected_phrase
                        )
                        ma_decision.token_corrected = self._capitalize_first_letter(
                            ma_decision.token_corrected
                        )
                        ma_decision.choices = [
                            {
                                **choice,
                            ...
                            for choice in ma_decision.choices
                        ]
                    is_ambiguous, is_crucial = token_choice_state(
                        ma_decision.choices,
                        force_crucial=ma_decision.force_crucial,
                    )
                    tokens.append(
                        {
                            "type": "phrase",
                            ...
                            "corrected": ma_decision.token_corrected,
                            "ambiguous": is_ambiguous,
                            "crucial": is_crucial,
                            "choices": ma_decision.choices,
                        }
                    )
                    corrected_parts.append(ma_decision.corrected_phrase)
                    previous_surface_word = ma_decision.previous_surface
                    last_end = matches[index + 1].end()
                    index += 2
                    continue
            ...
            if (
                ...
                next_corrected_for_ma = self.correct_word(next_word)
                next_is_verb_for_ma = self._is_verb_tagged_word(
                    next_corrected_for_ma
                )
                if (
                    current_norm in {"ma", "ma'"}
                    and self._normalize_word(previous_surface_word or "")
                    != "mingħajr"
                ) and not (
                    self._article_like_token(next_norm)
                    and ("-" in next_norm or index + 2 < len(matches))
                    and not next_is_verb_for_ma
                ):
                    ...
                    if ma_decision is not None:
                        ...
                        tokens.append(
                            {
                                "type": "phrase",
                                "original": ma_decision.token_original,
                                "corrected": ma_decision.token_corrected,
                                "ambiguous": is_ambiguous,
                                "crucial": is_crucial,
                                "choices": ma_decision.choices,
                ...
                if (
                    current_norm in {"min", "minn"}
                    and next_norm in {"habba", "ħabba"}
                ):
                    corrected_phrase = self._apply_surface_case(
                        original_word,
                        "minħabba",
                ...
                if current_norm in {
                    "il", "l", "lil", "lill", "lir", "lit", "lid", "lin",
                    "liċ", "lic", "lis", "liż", "liz", "lix"
                } and not next_norm.startswith("-") and not current_norm.endswith("-"):
                    corrected_next = self.correct_word(next_word)
                    corrected_next_norm = self._normalize_word(corrected_next)
                    if corrected_next_norm and not corrected_next_norm.startswith(
                        ("il-", "l-", "iċ-", "id-", "in-", "ir-", "is-", "it-", "ix-", "iz-", "iż-", "lil-", "lill-", "lir-", "lit-", "lid-", "lin-", "liċ-", "lis-", "liż-", "lix-")
                    ):
                        first_ch = self._graphemes(corrected_next_norm)[0]
                        vowels = set("aeiouàèìòùáéíóúâêîôû")
                        sun_letters = {
                            "ċ": "liċ", "c": "liċ",
                            "d": "lid",
                        ...
                        if current_norm in {"il", "l"}:
                            if first_ch in {"għ", "gh", "h"}:
                                fused_art = "l-" if current_norm == "l" else "il-"
                            elif first_ch in vowels:
                                fused_art = "l-"
                            elif first_ch in {"ċ", "c", "d", "n", "r", "s", "t", "x", "z", "ż"}:
                                fused_art = f"i{first_ch}-"
                            else:
                                fused_art = "il-"
                        else:
                            if first_ch in vowels:
                                fused_art = "lill-"
                            elif first_ch in sun_letters:
                                fused_art = f"{sun_letters[first_ch]}-"
                            else:
                                fused_art = "lil-"
                ...
                if current_norm in {"min", "minn"} and not (
                    ...
                    if min_decision is not None:
                        is_ambiguous, is_crucial = token_choice_state(
                            min_decision.choices,
                            force_crucial=min_decision.force_crucial,
                        )
                        tokens.append(
                            {
                                "type": "phrase",
                                "original": min_decision.token_original,
                                "corrected": min_decision.token_corrected,
                                "ambiguous": is_ambiguous,
                                "crucial": is_crucial,
                                "choices": min_decision.choices,
                            }
                        )
                        corrected_parts.append(min_decision.corrected_phrase)
                        previous_surface_word = min_decision.previous_surface
                        last_end = matches[index + 1].end()
                        index += 2
                        continue
            ...
            if index + 1 < len(matches) and not getattr(
                ...
                if (
                    ...
                    and current_norm not in {"x", "b", "f", "m", "t", "s"}
                    and next_norm
                ):
            ...
            if "-" in original_norm:
                article_prefix, article_tail = original_norm.split("-", 1)
                if article_prefix in {"il", "l", "iċ", "ic", "id", "in", "ir", "is", "it", "ix", "iż", "iz", "ċ", "c", "d", "n", "r", "s", "t", "x", "ż", "z"} and article_tail:
                    article_tail_corrected = self.correct_word(article_tail)
                    article_tail_norm = self._normalize_word(article_tail_corrected)
                    if (
                        article_tail_norm
                        and article_tail_norm != article_tail
                        and self._is_recognized_surface(article_tail_norm)
                    ):
                        corrected_word = f"{article_prefix}-{article_tail_norm}"
            if (
                not sentence_initial
                and self._is_initial_capitalized(original_word)
                and not original_word.isupper()
            ...
            pre_agreement_corrected = corrected_word
            corrected_word = self._agree_adjective_with_previous_noun(
                corrected_word,
                previous_surface_word,
            )
            adjective_agreement_applied = (
                self._normalize_word(pre_agreement_corrected)
            ...
            if self._normalize_word(original_word) == "hu":
                choices = []
            if adjective_agreement_applied:
                choices = []
            if (
                self._normalize_word(corrected_word) == self._normalize_word(original_word)
                and (self._is_recognized_surface(original_norm) or original_norm in self.dictionary_set)
                and original_norm not in self.EXACT_SUGGESTION_OVERRIDES
                and not (
                    hasattr(self, "orthographic_generator")
                    and self.orthographic_generator.dictionary_shortcut_variants(
            ...
            if (
                self._normalize_word(surface_word) == self._normalize_word(original_word)
                and (self._is_recognized_surface(original_norm) or original_norm in self.dictionary_set)
                and original_norm not in self.EXACT_SUGGESTION_OVERRIDES
                and not (
                    hasattr(self, "orthographic_generator")
        ...
        self._add_contextual_cluster_confusion_choices(tokens)
        corpus_context_selector = getattr(self, "corpus_context_selector", None)
        if corpus_context_selector is not None:
            corpus_context_selector.apply(tokens, self)
            corrected_text = self._ensure_terminal_period(
                "".join(
        ...
        self.meaning_for.cache_clear()
        for token in tokens:
            choices = self._clean_visible_choices_for_token(token) if isinstance(token, dict) else []
            if isinstance(token, dict):
                token["choices"] = choices
                if token.get("type") in {"word", "phrase"}:
                    token["ambiguous"] = len(choices) >= 2
                    if len(choices) < 2:
                        token["crucial"] = False
            if isinstance(token, dict) and choices:
                token_original_norm = self._normalize_word(str(token.get("original", "")))
                token_corrected_norm = self._normalize_word(str(token.get("corrected", "")))
                choice_norms = {
                    self._normalize_word(str(choice.get("word", "")))
                    for choice in choices
                    if isinstance(choice, dict)
                ...
                if (
                    token_original_norm == token_corrected_norm
                    and "-" in token_corrected_norm
                    and self._is_recognized_surface(token_corrected_norm)
                    and token_corrected_norm not in self.EXACT_SUGGESTION_OVERRIDES
                    and token_corrected_norm not in self.MANUAL_WORD_SUGGESTIONS
                    and not (
                        token_corrected_norm.startswith("l-")
                        and self._is_li_relative_adjective_allowed(
                            token_corrected_norm[2:]
                        )
                        and any(
                            self._normalize_word(str(choice.get("word", ""))).startswith("l'")
                            for choice in choices
                            if isinstance(choice, dict)
                        )
                    )
                ) or choice_norms == {token_corrected_norm}:
                    token["choices"] = []
            ...
            for choice in choices:
                if not isinstance(choice, dict):
                    continue
                if not str(choice.get("meaning", "")).strip():
