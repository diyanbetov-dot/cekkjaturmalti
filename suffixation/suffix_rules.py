...
class MalteseSuffixRules:
    ...
    def parse_suffixes(
        ...
        *,
        ...
        all_typed_endings = {ending for ending, _spec in ending_rows}
        ...
        for ending, spec in ending_rows:
            ...
            # A visible h belongs to the longest h-initial suffix. Shorter
            # aliases exist only to recover an omitted h; they must never
            # absorb a typed h into the lexical stem (amil+hulu, not amilh+ulu).
            if stem.endswith("h") and "h" + ending in all_typed_endings:
                continue
    ...
    def final_VC_to_iC(self, word: str) -> str | None:
        return self.final_eC_to_iC(word)
    ...
    def f1_final_weak_kaka_stem(self, record: VerbFormRecord) -> str | None:
        """Contract an F1 final-weak KaKa perfect before a direct object.

        qara -> qra-, dara -> dra-, faga -> fga-. The decision is based on
        the verb record and grapheme shape, not on a list of individual verbs.
        """
        graphemes = self.graphemes(record.word)
        if (
            record.is_f1
            and record.is_perf
            and self.verb_index.is_final_weak(record)
            and len(graphemes) == 4
            and self.is_consonant(graphemes[0])
            and graphemes[1] == "a"
            and self.is_consonant(graphemes[2])
            and graphemes[3] == "a"
        ):
            return self.from_graphemes([graphemes[0], graphemes[2], "a"])
        return None
    ...
    def stems_for_spec(
        ...
        final_weak_perfect_with_object = bool(
            record.is_perf
            and self.verb_index.is_final_weak(record)
            and base.endswith("a")
            and spec.kind in {"DO", "DO_IDO"}
        )
        ec_to_ic_required = bool(
            self.final_eC_to_iC(base)
            and (
                spec.kind in {"IDO", "DO_IDO"}
                or spec.label in {"DO_1S", "DO_3SF", "DO_1P", "DO_2P", "DO_3P"}
            )
        )
        ido_syncope_required = bool(
            spec.kind == "IDO"
            and self.ends_vowel_consonant(base)
            and not base.endswith("'")
            and root_class != "HOLLOW"
        )
        ...
        elif not final_weak_perfect_with_object and not ec_to_ic_required and not ido_syncope_required:
        ...
        if is_medial and record.is_mperf and spec.kind in {"DO", "IDO", "DO_IDO"}:
        ...
        if self.verb_index.is_final_weak(record):
            ...
            elif spec.kind in {"DO", "DO_IDO"} and record.is_perf:
                kaka_stem = self.f1_final_weak_kaka_stem(record)
                if kaka_stem is not None:
                    self.add_unique(
                        stems,
                        kaka_stem,
                        "F1_FINAL_WEAK_KAKA_CONTRACTION",
                        "F1 final-weak KaKa -> KKa before direct object suffix",
                    )
                elif record.is_f1:
                    self.add_unique(
                        stems,
                        self.final_VCa_to_Cie(base),
                        "F1_FINAL_WEAK_VCA_TO_CIE",
                        "F1 final-weak VCa -> Cie before direct object suffix",
                    )
                else:
                    self.add_unique(
                        stems,
